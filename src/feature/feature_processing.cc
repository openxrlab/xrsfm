...
void SiftMatch(const FeatureDescriptors &descs1,
               const FeatureDescriptors &descs2, std::vector<Match> &matches,
...
void OrbMatch(const cv::Mat &descs1, const cv::Mat &descs2,
    ...
    Eigen::Matrix<int, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> dists(
        descs1.rows, descs2.rows);
    for (int i1 = 0; i1 < descs1.rows; ++i1) {
        for (int i2 = 0; i2 < descs2.rows; ++i2) {
            dists(i1, i2) =
                OrbDistance(descs1.data + i1 * 32, descs2.data + i2 * 32);
        }
    }
    // find matches
    matches.resize(0);
    std::vector<int> map12(descs1.rows, -1), map21(descs2.rows, -1);
    std::vector<int> dist12(descs1.rows, max_dist),
        dist21(descs2.rows, max_dist);
    std::vector<int> sdist12(descs1.rows, max_dist),
        sdist21(descs2.rows, max_dist);
    for (int i1 = 0; i1 < descs1.rows; ++i1) {
        for (int i2 = 0; i2 < descs2.rows; ++i2) {
    ...
    for (int i1 = 0; i1 < descs1.rows; ++i1) {
        if (map12[i1] == -1 || dist12[i1] > distance_th ||
            map21[map12[i1]] != i1 || dist12[i1] > sdist12[i1] * max_ratio)
            continue;
...
void FeatureMatching(const std::vector<Frame> &frames,
    ...
    for (const auto &[id1, id2] : candidate_pairs) {
        ...
        SiftMatch(frame1.uint_descs_, frame2.uint_descs_, frame_pair.matches,
                  &sift_match_gpu);
#else
...
#pragma omp parallel for schedule(static, 8)
    for (int i = 0; i < frame_pairs.size(); ++i) {
        ...
        const int inlier_threshold =
            std::max(min_num_inlier,
                     (int)(min_ratio_inlier * frame_pair.matches.size()));
        if (frame_pair.inlier_num < inlier_threshold) {
            frame_pair.inlier_num = 0;
            continue;
        }
        // filter the outlier matches
        std::vector<Match> inliner_matches = ExtractInlierMatches(
            frame_pair.matches, frame_pair.inlier_num, frame_pair.inlier_mask);
    ...
    std::cout << "matched image pairs: " << count_inlier_pairs << "/"
              << candidate_pairs.size() << std::endl;
}
...
void ExpansionAndMatching(Map &map,
    ...
    std::cout << "init: " << map.frame_pairs_.size() << " "
              << init_candidates.size() << std::endl;
    std::vector<std::set<int>> id2matched_ids(map.NumFrames());
    ...
    for (int i = 0; i < num_iteration; i++) {
        ...
        match_expansion_solver.SetUp(map, id2rank, images, ini_frame1,
                                     ini_frame2);
        ...
        TIMING(timer_matching,
               FeatureMatching(map.frames_, cur_candidats, map.frame_pairs_,
                               b_use_fundamental));

        std::cout << "expansion: " << cur_candidats.size() << " "
                  << map.frame_pairs_.size() << " " << all_candidates.size()
                  << std::endl;
        std::cout << "precision: "
                  << 100.0 * map.frame_pairs_.size() / all_candidates.size()
                  << std::endl;
