...
inline Eigen::Matrix3d jr(Eigen::Vector3d theta) {
    ...
    if (norm < 1.745329252e-7) {
        jr = Eigen::Matrix3d::Identity() - 1.0 / 2.0 * skewSymmetric(theta) + 1.0 / 6.0 * skewSymmetric(theta) * skewSymmetric(theta);
    } else {
        jr = Eigen::Matrix3d::Identity() - (1.0 - cos(norm)) / (norm * norm) * skewSymmetric(theta) + (norm - sin(norm)) / (norm * norm * norm) * skewSymmetric(theta) * skewSymmetric(theta);
    }
    return jr;
}
...
inline Eigen::Matrix3d jri(Eigen::Vector3d theta) {
    ...
    Eigen::Matrix3d jri;
    if (norm < 1.745329252e-7) {
        jri = Eigen::Matrix3d::Identity() + 1.0 / 2.0 * skewSymmetric(theta) + 1.0 / 12.0 * skewSymmetric(theta) * skewSymmetric(theta);
    } else {
        jri = Eigen::Matrix3d::Identity() + 1.0 / 2.0 * skewSymmetric(theta) + ((1.0) / (norm * norm) - (1.0 + cos(norm)) / (2 * norm * sin(norm))) * skewSymmetric(theta) * skewSymmetric(theta);
    }
    return jri;
}
} // namespace xrsfm
