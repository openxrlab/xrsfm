...
bool CheckInitFramePair(const Map &map, FramePair &frame_pair,
    ...
    xrsfm::decompose_rt(frame_pair.E, points1, points2, frame_pair.R,
                        frame_pair.t, point3ds, inlier_mask);
    ...
    if (num_p3d >= 0.5 * id_p3d && num_p3d_valid >= 0.5 * num_p3d &&
        num_p3d_valid > 200) {
        printf("id %d-%d: %d %d %d\n", frame_pair.id1, frame_pair.id2,
               num_p3d_valid, num_p3d, id_p3d);
        return true;
    }
    return false;
}
...
bool FindInitFramePair(const Map &map, FramePair &init_frame_pair) {
    ...
    for (const auto &init_id1 : init_id1_candidate) {
        std::vector<int> init_id2_candidate;
        {
            ...
            for (const auto &corrs :
                 map.corr_graph_.frame_node_vec_[init_id1].corrs_vector) {
    ...
    for (size_t i = 0; i < num_matches; ++i) {
        if (frame_pair.inlier_mask[i]) { // assert track valid
            if (inlier_mask[id_p3d]) {
                ...
                frame1.track_ids_[ftr_id1] = frame2.track_ids_[ftr_id2] =
                    track_id;
