...
inline matrix<9, 4>
generate_nullspace_basis(const std::array<vector2, 5> &points1,
                         const std::array<vector2, 5> &points2) {
    matrix<5, 9> A;
    for (size_t i = 0; i < 5; ++i) {
        matrix3 h = vector3(points1[i].homogeneous()) *
...
double sampson_error(const matrix3 &E, const vector2 &p1, const vector2 &p2) {
    ...
    return r * r *
           (1 / Ep1.segment<2>(0).squaredNorm() +
            1 / Etp2.segment<2>(0).squaredNorm());
}

std::vector<matrix3>
solve_essential_5pt(const std::array<vector2, 5> &points1,
                    const std::array<vector2, 5> &points2) {
...
matrix3 find_essential_matrix(const std::vector<vector2> &points1,
    ...
    matrix3 best_E =
        matrix3::Constant(std::numeric_limits<double>::quiet_NaN());
    ...
    for (size_t iter = 0; iter < iter_max; ++iter) {
        ...
        for (size_t i = 0; i < Es.size(); ++i) {
            ...
            if (inlier > best_inlier ||
                (inlier == best_inlier && best_inlier > 0 &&
                 score < best_score)) {
                best_E = E;
                best_inlier = inlier;
                best_score = score;
...
bool check_point(const matrix<3, 4> P1, const matrix<3, 4> P2,
    ...
    if (q1[2] * q[3] > 0 && q2[2] * q[3] > 0 && q1[2] / q[3] < 100 &&
        q2[2] / q[3] < 100) {
        p = q.hnormalized();
        return true;
    }
...
void decompose_rt(const matrix3 &E, const std::vector<vector2> &points1,
    ...
    int best_k = 0;
    for (int k = 0; k < 4; ++k) {
        int goods = check_essential_rt(points1, points2, Rs[k], Ts[k],
                                       points_array[k], status_array[k]);
        if (goods > best_num) {
            best_num = goods;
            best_k = k;
        }
