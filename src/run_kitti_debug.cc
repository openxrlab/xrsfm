...
void PreProcess(const std::string dir_path, Map& map) {
  ...
  LoadImageNames(dir_path+"/image_0/", image_names);
  ReadFeatures(dir_path + "/open/nv50ftr.bin", frames, true);
  ReadFramePairs(dir_path + "/open/nv50fp.bin", frame_pairs);  
...
int main(int argc, char* argv[]) {
  ...
  for (int iter = 0; iter < map.frames_.size(); iter++) {
    if(iter%10==0)viewer.update_map(map);
    // viewer.Draw(map,false);
    // if(iter==1000)SaveSfMState(output_path + seq_name + "/1000.bin", iter, map);
    // if(iter==0){
    //   LoadSfMState(output_path + seq_name + "/1000.bin", iter, map);
  ...
  WriteColMapDataBinary(output_path + seq_name + "/open/", map);
