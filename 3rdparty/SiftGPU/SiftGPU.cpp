...
inline void SiftGPU::InitSiftGPU() {
    ...
    if (GlobalUtil::_UseCUDA) {
        ...
        std::cerr << "---------------------------------------------------------"
                     "------------------\n"
                  << "CUDA not supported in this binary! To enable it, please "
                     "use SiftGPU_CUDA_Enable\n"
                  << "solution for VS2005+ or set siftgpu_enable_cuda to 1 in "
                     "makefile\n"
                  << "---------------------------------------------------------"
                     "-------------------\n";
    }
    ...
    if (GlobalUtil::_UseCUDA &&
        !PyramidCU::CheckCudaDevice(GlobalUtil::_DeviceIndex)) {
    ...
    if (GlobalUtil::_GoodOpenGL && GlobalUtil::_InitPyramidWidth > 0 &&
        GlobalUtil::_InitPyramidHeight > 0) {
        GlobalUtil::StartTimer("Initialize Pyramids");
        _pyramid->InitPyramid(GlobalUtil::_InitPyramidWidth,
                              GlobalUtil::_InitPyramidHeight, 0);
...
int SiftGPU::RunSIFT(int width, int height, const void *data,
    ...
    if (width > 0 && height > 0 && data != NULL) {
        ...
        if (_texImage->SetImageData(width, height, data, gl_format, gl_type)) {
            ...
            // if the size of image is different, the pyramid need to be
            // reallocated.
...
int SiftGPU::RunSIFT(int num, const SiftKeypoint *keys,
                     int keys_have_orientation) {
    if (num <= 0)
        return 0;
    _pyramid->SetKeypointList(num, (const float *)keys, 1,
                              keys_have_orientation);
    return RunSIFT();
}
...
void SiftGPU::SetKeypointList(int num, const SiftKeypoint *keys,
                              int keys_have_orientation) {
    _pyramid->SetKeypointList(num, (const float *)keys, 0,
                              keys_have_orientation);
}
...
void SiftGPUEX::DisplayOctave(void (*UseDisplayShader)(), int i) {
    ...
    for (int level = _level_min; level <= _level_max; level++) {
        GLTexImage *tex =
            _pyramid->GetLevelTexture(i + _pyramid->_octave_min, level);
...
void SiftGPUEX::DisplayPyramid(void (*UseDisplayShader)(), int dataName,
    ...
    for (int i = _pyramid->_octave_min;
         i < _pyramid->_octave_min + _pyramid->_octave_num; i++) {
...
void SiftGPUEX::DisplaySIFT() {
    ...
    case 0:
        ...
        break;
    case 1:
        DisplayPyramid(ShaderMan::UseShaderDisplayGaussian,
                       SiftPyramid::DATA_GAUSSIAN);
        break;
    case 2:
    ...
    case 3:
        ...
        break;
    case 4:
        DisplayPyramid(ShaderMan::UseShaderDisplayDOG, SiftPyramid::DATA_DOG,
                       1);
        break;
    case 5:
        DisplayPyramid(ShaderMan::UseShaderDisplayGrad, SiftPyramid::DATA_GRAD,
                       1);
        break;
    case 6:
        DisplayPyramid(ShaderMan::UseShaderDisplayDOG, SiftPyramid::DATA_DOG, 2,
                       1);
        DisplayPyramid(ShaderMan::UseShaderDisplayKeypoints,
                       SiftPyramid::DATA_KEYPOINT, 2, 1);
    }
}

void SiftGPUEX::SetView(int view, int sub_view, char *title) {
    const char *view_titles[] = {
        "Original Image", "Gaussian Pyramid",       "Octave Images",
        "Level Image",    "Difference of Gaussian", "Gradient",
        "Keypoints"};
...
void SiftGPU::PrintUsage() {
    std::cout
        ...
        << "-f <float>        : Filter width factor; Width will be 2*factor+1 "
           "(default : 4.0)\n"
        << "-w  <float>       : Orientation sample window factor (default: "
           "2.0)\n"
        << "-dw <float>  *    : Descriptor grid size factor (default : 3.0)\n"
        << "-fo <int>    *    : First octave to detect DOG keypoints(default : "
           "0)\n"
        << "-no <int>         : Maximum number of Octaves (default : no "
           "limit)\n"
        << "-d <int>          : Number of DOG levels in an octave (default : "
           "3)\n"
        ...
        << "-s  <int=1>       : Sub-Pixel, Sub-Scale Localization, "
           "Multi-Refinement(num)\n"
        << "-lcpu -lc <int>   : CPU/GPU mixed Feature List Generation "
           "(default: 6)\n"
        << "                    Use GPU first, and use CPU when reduction size "
           "<= pow(2,num)\n"
        << "                    When <num> is missing or equals -1, no GPU "
           "will be used\n"
        << "-noprep           : Upload raw data to GPU (default: RGB->LUM and "
           "down-sample on CPU)\n"
        ...
        << "-cuda <int=0>     : Use CUDA SiftGPU, and specify the device "
           "index\n"
        << "-tight            : Automatically resize pyramid to fit new images "
           "tightly\n"
        << "-p  <W>x<H>       : Inititialize the pyramids to contain image of "
           "WxH (eg -p 1024x768)\n"
        << "-tc[1|2|3] <int> *: Threshold for limiting the overall number of "
           "features (3 methods)\n"
        << "-v <int>          : Level of timing details. Same as calling "
           "Setverbose() function\n"
        << "-loweo            : (0, 0) at center of top-left pixel (default: "
           "corner)\n"
        << "-maxd <int> *     : Max working dimension (default : 2560 "
           "(unpacked) / 3200 (packed))\n"
        << "-nomc             : Disabling auto-downsamping that try to fit GPU "
           "memory cap\n"
        << "-exit             : Exit program after processing the input image\n"
        << "-unpack           : Use the old unpacked implementation\n"
        << "-di               : Use dynamic array indexing if available "
           "(default : no)\n"
        << "                    It could make computation faster on cards like "
           "GTX 280\n"
        << "-ofix     *       : use 0 as feature orientations.\n"
        << "-ofix-not *       : disable -ofix.\n"
        << "-winpos <X>x<Y> * : Screen coordinate used in Win32 to select "
           "monitor/GPU.\n"
        << "-display <string>*: Display name used in Linux/Mac to select "
           "monitor/GPU.\n"
        << "\n"
        << "NOTE: parameters marked with * can be changed after "
           "initialization\n"
        << "\n";
}
    ...
    for (int i = 0; i < argc; i++) {
        ...
        case MAKEINT4(c, u, d, a):
            ...
            if (!_initialized) {
                GlobalUtil::_UseCUDA = 1;
                int device = -1;
                if (i + 1 < argc && sscanf(param, "%d", &device) &&
                    device >= 0) {
            ...
            std::cerr << "-----------------------------------------------------"
                         "----------------------\n"
                      << "CUDA not supported in this binary! To enable it, "
                         "please use SiftGPU_CUDA_Enable\n"
                      << "solution for VS2005+ or set siftgpu_enable_cuda to 1 "
                         "in makefile\n"
                      << "-----------------------------------------------------"
                         "-----------------------\n";
#endif
        ...
        case MAKEINT2(c, l):
            ...
            std::cerr << "-----------------------------------------------------"
                         "----------------------\n"
                      << "OpenCL not supported in this binary! Define "
                         "CL_CUDA_SIFTGPU_ENABLED to..\n"
                      << "-----------------------------------------------------"
                         "-----------------------\n";
#endif
            break;
...
float SiftParam::GetLevelSigma(int lev) {
    return _sigma0 *
           powf(2.0f, float(lev) / float(_dog_level_num)); // bug fix 9/12/2007
}
...
void SiftGPUEX::ToggleDisplayDebug() { _view_debug = !_view_debug; }
...
int SiftGPU::GetFeatureNum() { return _pyramid->GetFeatureNum(); }
...
int SiftGPU::AllocatePyramid(int width, int height) {
    ...
    return _pyramid->_pyramid_height == height &&
...
int SiftGPU::GetMaxOrientation() { return GlobalParam::_MaxOrientation; }

int SiftGPU::GetMaxDimension() { return GlobalUtil::_texMaxDim; }

int SiftGPU::GetImageCount() { return _list->size(); }
