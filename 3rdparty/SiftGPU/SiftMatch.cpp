...
void SiftMatchGL::AllocateSiftMatch() {
    ...
    if (__max_sift > GlobalUtil::_texMaxDimGL)
        __max_sift = GlobalUtil::_texMaxDimGL;
    ///
    ...
    _texDes[0].InitTexture(_sift_per_row * _pixel_per_sift,
                           __max_sift / _sift_per_row, 0, GL_RGBA8);
    _texDes[1].InitTexture(_sift_per_row * _pixel_per_sift,
                           __max_sift / _sift_per_row, 0, GL_RGBA8);
...
void SiftMatchGL::SetDescriptors(int index, int num,
    ...
    if (_sift_num_stripe == 1) {
        glTexSubImage2D(GlobalUtil::_texTarget, 0, 0, 0, w, h, GL_RGBA,
                        GL_UNSIGNED_BYTE, &sift_buffer[0]);
    } else {
        for (int i = 0; i < _sift_num_stripe; ++i) {
            int ws = _sift_per_stripe * _pixel_per_sift;
            int x = i * ws;
            int pos = i * ws * h * 4;
            glTexSubImage2D(GlobalUtil::_texTarget, 0, x, 0, ws, h, GL_RGBA,
                            GL_UNSIGNED_BYTE, &sift_buffer[pos]);
        }
    }
    _texDes[index].UnbindTex();
}

void SiftMatchGL::SetFeautreLocation(int index, const float *locations,
                                     int gap) {
    ...
    if (gap == 0) {
        memcpy(&sift_buffer[0], locations,
               _num_sift[index] * 2 * sizeof(float));
    } else {
    ...
    if (_sift_num_stripe == 1) {
        glTexSubImage2D(GlobalUtil::_texTarget, 0, 0, 0, w, h,
                        GL_LUMINANCE_ALPHA, GL_FLOAT, &sift_buffer[0]);
    } else {
        for (int i = 0; i < _sift_num_stripe; ++i) {
            int ws = _sift_per_stripe;
            int x = i * ws;
            int pos = i * ws * h * 2;
            glTexSubImage2D(GlobalUtil::_texTarget, 0, x, 0, ws, h,
                            GL_LUMINANCE_ALPHA, GL_FLOAT, &sift_buffer[pos]);
        }
    }
...
void SiftMatchGL::SetDescriptors(int index, int num, const float *descriptors,
                                 int id) {
    if (_initialized == 0)
        return;
    if (index > 1)
...
void SiftMatchGL::LoadSiftMatchShadersGLSL() {
    ...
    out << "#define SIFT_PER_STRIPE " << _sift_per_stripe
        ...
        << "   vec4 val = vec4(0.0, 0.0, 0.0, 0.0), data1, buf;\n"
           ...
           "   vec2 index_h = vec2(SIFT_PER_STRIPE)* (temp_div2 - "
           "temp_floor2);\n"
           "   vec2 tx = (index_h + stripe_index * vec2(SIFT_PER_STRIPE))* "
           "vec2(PIXEL_PER_SIFT) + 0.5;\n"
           "   vec2 tpos1, tpos2; \n"
    ...
    out << "#define SIFT_PER_STRIPE " << _sift_per_stripe
        ...
        << "   vec4 val = vec4(0.0, 0.0, 0.0, 0.0), data1, buf;\n"
           ...
           "   vec4 tlpos = vec4((index_h + stripe_index * "
           "vec2(SIFT_PER_STRIPE)) + 0.5, index_v);\n"
           ...
           "   if(disth > size.z ) {gl_FragColor = vec4(0.0, index, 0.0); "
           "return;}\n"
           ...
           "   if(sampson_error > size.w) {gl_FragColor = vec4(0.0, index, "
           "0.0); return;}\n"

           // compare feature descriptor
           "   vec2 tx = (index_h + stripe_index * SIFT_PER_STRIPE)* "
           "vec2(PIXEL_PER_SIFT) + 0.5;\n"
           "   vec2 tpos1, tpos2; \n"
           "	vec4 tpos = vec4(tx, index_v);\n"
           "   for(int i = 0; i < PIXEL_PER_SIFT; ++i){\n"
    ...
    out << "#define BLOCK_WIDTH 16.0\n"
           ...
           "void main ()\n"
           "{\n"
           "	float index = gl_FragCoord.x + floor(gl_FragCoord.y) * "
           "BLOCK_WIDTH; \n"
           ...
           "		bestv  = v > bestv.r? vec2(v, bestv.r) : max(bestv, "
           "vec2(v));\n "
           "	}\n"
           "	bestv = acos(min(bestv, 1.0));\n"
           "	if(bestv.x >= param.y || bestv.x >= param.z * bestv.y) imax = "
           "-1.0;\n"
...
int SiftMatchGL::GetGuidedSiftMatch(int max_match, uint32_t match_buffer[][2],
    ...
    // set parameters glsl
    float dot_param[4] = {(float)_texDes[0].GetDrawHeight(),
                          (float)_texDes[1].GetDrawHeight(), hdistmax,
                          fdistmax};
...
int SiftMatchGL::GetBestMatch(int max_match, uint32_t match_buffer[][2],
    ...
    if (mbm) {
        ...
        glUniform3f(_param_rowmax_param, (float)_num_sift[0], distmax,
                    ratiomax);
        _texMatch[1].DrawQuad();
        glReadPixels(0, 0, 16, (_num_sift[1] + 15) / 16, GL_RED, GL_FLOAT,
                     buffer2);
    }
...
int SiftMatchGL::GetSiftMatch(int max_match, uint32_t match_buffer[][2],
    ...
    // set parameters
    float heights[2] = {(float)_texDes[0].GetDrawHeight(),
                        (float)_texDes[1].GetDrawHeight()};
...
int SiftMatchGPU::_VerifyContextGL() {
    ...
    if (__language >= SIFTMATCH_CUDA) {
    } else if (__language == SIFTMATCH_SAME_AS_SIFTGPU &&
               GlobalUtil::_UseCUDA) {
    } else
        GlobalUtil::InitGLParam(0);
    if (GlobalUtil::_GoodOpenGL == 0)
    ...
    if (((__language == SIFTMATCH_SAME_AS_SIFTGPU && GlobalUtil::_UseCUDA) ||
         __language >= SIFTMATCH_CUDA) &&
    ...
    if ((__language == SIFTMATCH_SAME_AS_SIFTGPU && GlobalUtil::_UseCUDA) ||
        ...
        std::cerr << "---------------------------------------------------------"
                  ...
                  << "Project for VS2005+ or set siftgpu_enable_cuda to 1 in "
                     "makefile\n"
                  << "---------------------------------------------------------"
                     "-------------------\n";
    }
    ...
    if (GlobalUtil::_verbose)
        std::cout << "[SiftMatchGPU]: "
                  << (__language == SIFTMATCH_CUDA ? "CUDA" : "GLSL") << "\n\n";
...
void SiftMatchGPU::SetFeautreLocation(int index, const float *locations,
                                      int gap) {
