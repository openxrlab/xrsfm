...
class SiftGPU : public SiftParam {
  ...
  protected:
    ...
    int _current;
    //_initialized indicates if the shaders and OpenGL/SIFT parameters are
    // initialized they are initialized only once for one SiftGPU inistance that
    // is, SIFT parameters will not be changed
    int _initialized;
  ...
  public:
    ...
    SIFTGPU_EXPORT virtual void GetFeatureVector(SiftKeypoint *keys,
                                                 float *descriptors);
    // Set keypoint list before running sift to get descriptors
    SIFTGPU_EXPORT virtual void SetKeypointList(int num,
    ...
    //(for example, you call wglmakecurrent yourself and verify the current
    // context)
    ...
    // gl_type (e.g. GL_UNSIGNED_BYTE, GL_FLOAT) is the data type of the pixel
    // data; Check glTexImage2D(...format, type,...) for the accepted values
    // Using image data of GL_LUMINANCE + GL_UNSIGNED_BYTE can minimize transfer
    // time
    SIFTGPU_EXPORT virtual int RunSIFT(int width, int height, const void *data,
                                       unsigned int gl_format,
                                       unsigned int gl_type);
    // run SIFT on current image (specified by arguments), or processing the
    // current image again
...
class SiftGPUEX : public SiftGPU {
    ...
    void DisplayLevel(void (*UseDisplayShader)(),
                      int i); // display one level image
    void DisplayOctave(void (*UseDisplayShader)(),
...
class SiftMatchGPU {
  public:
    enum SIFTMATCH_LANGUAGE {
        ...
        SIFTMATCH_CUDA_DEVICE0 =
            3 // to use device i, use SIFTMATCH_CUDA_DEVICE0 + i
    };
  ...
  public:
    // OpenGL Context creation/verification, initialization is done
    ...
    SetDescriptors(int index, int num, const float *descriptors, int id = -1);
    // Option 2 unsigned char descriptors. They must be already normalized to
    // 512
    ...
    SIFTGPU_EXPORT virtual int
    GetSiftMatch(int max_match,              // the length of the match_buffer.
                 uint32_t match_buffer[][2], // buffer to receive the matched
                                             // feature indices
    ...
    SIFTGPU_EXPORT virtual void
    SetFeautreLocation(int index, const float *locations, int gap = 0);
    inline void SetFeatureLocation(int index,
...
// connected 			multiple-GPU can be used by creating multiple
// instances 			GPU selection done through SiftGPU::ParseParam
// function otherwise, 			Assumes the existenc of a remote server
// and connects to it 			GPU selection skipped if already done on
// the server-end 			RUN server: server_siftgpu -server port
//[siftgpu_param] example: 	ComboSiftGPU * combo = CreateRemoteSiftGPU(7777,
//"my.gpuserver.com"); 	SiftGPU* siftgpu = combo, SiftMatchGPU * matcher =
// combo;
