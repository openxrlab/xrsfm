...
GLuint GlobalParam::_texTarget =
    GL_TEXTURE_RECTANGLE_ARB;                     // only this one is supported
...
int GlobalParam::_MaxFilterWidth =
    -1; // maximum filter width, use when GPU is not good enough
float GlobalParam::_FilterWidthFactor =
    4.0f; // the filter size will be _FilterWidthFactor*sigma*2+1
float GlobalParam::_DescriptorWindowFactor =
    3.0f; // descriptor sampling window factor
int GlobalParam::_SubpixelLocalization =
    1; // sub-pixel and sub-scale localization
int GlobalParam::_MaxOrientation =
    2; // whether we find multiple orientations for each feature
int GlobalParam::_OrientationPack2 =
    0; // use one float to store two orientations
float GlobalParam::_MaxFeaturePercent = 0.005f; // at most 0.005 of all pixels
int GlobalParam::_MaxLevelFeatureNum =
    4096;                              // maximum number of features of a level
int GlobalParam::_FeatureTexBlock = 4; // feature texture storagte alignment
int GlobalParam::_NarrowFeatureTex = 0;

// if _ForceTightPyramid is not 0, pyramid will be reallocated to fit the size
// of input images. otherwise, pyramid can be reused for smaller input images.
...
int GlobalParam::_PreProcessOnCPU =
    1; // convert rgb 2 intensity on gpu, down sample on GPU

// hardware parameter,   automatically retrieved
int GlobalParam::_texMaxDim =
    3200; // Maximum working size for SiftGPU, 3200 for packed
...
int GlobalParam::_KeepExtremumSign =
    0; // if 1, scales of dog-minimum will be multiplied by -1
///
...
bool GlobalUtil::CheckFramebufferStatus() {
    ...
    case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_EXT:
        std::cerr << ("Framebuffer incomplete,attached images must have same "
                      "dimensions\n");
        return false;
...
int ClockTimer::ClockMS() { return 0; }
...
float ClockTimer::GetElapsedTime() { return 0; }
    ...
    if (NotTargetGL && !GlobalUtil::_UseSiftGPUEX) {
        ...
        if (vendor) {
            ...
            if (GlobalUtil::_IsNvidia)
                ...
                // loopes are too large, and it may take too much time to
                // compile
                GlobalUtil::_KeepShaderLoop = 1;
            }
#endif
            ...
            if (GlobalUtil::_IsNvidia &&
                ...
                glGetIntegerv(
                    0x9049 /*GL_GPU_MEM_INFO_CURRENT_AVAILABLE_MEM_NVX*/,
                    &_MemCapGPU);
                _MemCapGPU /= (1024);
...
void GlobalUtil::SelectDisplay() {
    ...
    _WindowDisplay = NULL;
    if (hdc == NULL) {
        std::cout << "ERROR: invalid display specified\n";
        return;
    }
...
int GlobalUtil::CreateWindowEZ(LiteWindow *window) {
    ...
    if (window->IsValid()) {
        ...
        std::cerr << "For nVidia cards, you can try change to CUDA mode in "
                     "this case\n";
        return 0;
    }
}
