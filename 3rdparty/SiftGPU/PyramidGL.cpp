...
PyramidNaive::~PyramidNaive() { DestroyPyramidData(); }
...
void PyramidGL::GetTextureStorageSize(int num, int &fw, int &fh) {
    if (num <= 0) {
        ...
        double dn = double(num);
        int nb = (int)ceil(dn / GlobalUtil::_texMaxDim /
                           GlobalUtil::_FeatureTexBlock);
...
void PyramidNaive::FitPyramid(int w, int h) {
    ...
    for (int i = 0; i < _octave_num; i++) {
        ...
        for (int j = param._level_min; j <= param._level_max;
             j++, tex++, aux++) {
...
void PyramidNaive::InitPyramid(int w, int h, int ds) {
    ...
    while (GlobalUtil::_MemCapGPU > 0 && GlobalUtil::_FitMemoryCap &&
           (wp > _pyramid_width || hp > _pyramid_height) &&
           max(max(wp, hp), max(_pyramid_width, _pyramid_height)) >
               1024 * sqrt(GlobalUtil::_MemCapGPU / 140.0)) {
        _octave_min++;
        wp >>= 1;
        hp >>= 1;
        toobig = 2;
    ...
    if (toobig && GlobalUtil::_verbose) {
        std::cout
            << (toobig == 2
                    ...
                    : "[**SKIP OCTAVES**]:\tReaching the dimension limit "
                      "(-maxd)!\n");
    }
    ...
    ShaderMan::SelectInitialSmoothingFilter(_octave_min + _down_sample_factor,
                                            param);
}
...
int PyramidGL::ResizeFeatureStorage() {
    ...
    std::fill(_levelFeatureNum,
              _levelFeatureNum + _octave_num * param._dog_level_num, 0);
    ...
    if (_histo_buffer == NULL)
        _histo_buffer =
            new float[((size_t)1) << (2 + 2 * GlobalUtil::_ListGenSkipGPU)];
    ...
    for (i = 0; i < _octave_num; i++) {
        GLTexImage *tex = GetBaseLevel(i + _octave_min);
        int fmax = int(tex->GetImgWidth() * tex->GetImgHeight() *
                       GlobalUtil::_MaxFeaturePercent);
        int fw, fh;
        //
        ...
        totalkb += fw * fh * 16 * param._dog_level_num *
                   (_orientationTex ? 2 : 1) / 1024;
    }
    ...
    if (_descriptorTex == NULL) {
        ...
        if (GlobalUtil::_DescriptorPPT) {
            ...
            ns = max(4,
                     GlobalUtil::_DescriptorPPT); // how many point in one
                                                  // texture for one descriptor
        } else {
        ...
        int nf =
            GlobalUtil::_DescriptorPPT ? 32 / GlobalUtil::_DescriptorPPT : 1;
        totalkb += nf * _descriptorTex[0].GetTexWidth() *
                   _descriptorTex[0].GetTexHeight() * 16 / 1024;
    }
    return totalkb;
}
...
void PyramidNaive::BuildPyramid(GLTexInput *input) {
    ...
    for (int i = _octave_min; i < _octave_min + _octave_num; i++) {
        ...
        for (int j = param._level_min + 1; j <= param._level_max;
             j++, tex++, filter++) {
            // filtering
...
GLTexImage *PyramidNaive::GetLevelTexture(int octave, int level, int dataName) {
    ...
    case DATA_ROT:
        return _texPyramid +
               (_pyramid_octave_first + octave - _octave_min) *
                   param._level_num +
...
GLTexImage *PyramidNaive::GetLevelTexture(int octave, int level) {
    return _texPyramid +
           (_pyramid_octave_first + octave - _octave_min) * param._level_num +
           (level - param._level_min);
}
...
GLTexImage *PyramidNaive::GetBaseLevel(int octave, int dataName) {
    ...
    case DATA_GRAD:
    case DATA_ROT:
        return _texPyramid + (_pyramid_octave_first + octave - _octave_min) *
                                 param._level_num;
    case DATA_KEYPOINT:
        return _auxPyramid + (_pyramid_octave_first + octave - _octave_min) *
                                 param._level_num;
    default:
        return NULL;
    }
...
void PyramidNaive::DetectKeypointsEX() {
    ...
    for (i = _octave_min; i < _octave_min + _octave_num; i++) {
        ...
        for (j = param._level_min + 2; j < param._level_max;
             j++, aux++, tex++) {
            if (GlobalUtil::_timingL)
                t = CLOCK();
            tex->AttachToFBO(0);
            ...
            (tex - 1)->BindTex();
            ShaderMan::UseShaderKeypoint((tex + 1)->GetTexID(),
                                         (tex - 1)->GetTexID());
            aux->DrawQuadMT8();
...
void PyramidNaive::GenerateFeatureList(int i, int j) {
    ...
    tex->BindTex();
    htex->AttachToFBO(0);
    int tight = ((htex->GetImgWidth() * 2 == tex->GetImgWidth() - 1 ||
                  tex->GetTexWidth() == tex->GetImgWidth()) &&
                 (htex->GetImgHeight() * 2 == tex->GetImgHeight() - 1 ||
                  tex->GetTexHeight() == tex->GetImgHeight()));
    ShaderMan::UseShaderGenListInit(tex->GetImgWidth(), tex->GetImgHeight(),
                                    tight);
    htex->FitTexViewPort();
    ...
    if (hist_skip_gpu == 0) {
        ...
        ShaderMan::UseShaderGenListStart((float)ftex->GetImgWidth(),
                                         htex->GetTexID());
        ...
        for (int y = 0, pos = 0; y < th; y++) {
            for (int x = 0; x < tw; x++) {
                for (int c = 0; c < 4; c++, pos++) {
                    ...
                    if (ss == 0)
                        continue;
                    float ft[4] = {2 * x + (c % 2 ? 1.5f : 0.5f),
                                   2 * y + (c >= 2 ? 1.5f : 0.5f), 0, 1};
                    for (int t = 0; t < ss; t++) {
                        ft[2] = (float)t;
                        _keypoint_buffer.insert(_keypoint_buffer.end(), ft,
                                                ft + 4);
                    }
                    fc += (int)ss;
                }
        ...
        _keypoint_buffer.resize(
            ftex->GetDrawWidth() * ftex->GetDrawHeight() * 4, 0);
        ///////////
        glActiveTexture(GL_TEXTURE0);
        ftex->BindTex();
        glTexSubImage2D(GlobalUtil::_texTarget, 0, 0, 0, ftex->GetDrawWidth(),
                        ftex->GetDrawHeight(), GL_RGBA, GL_FLOAT,
                        &_keypoint_buffer[0]);
        htex += 2;
    }
...
void PyramidNaive::GenerateFeatureList() {
    ...
    FOR_EACH_OCTAVE(i, reverse) {
        ...
        FOR_EACH_LEVEL(j, reverse) {
            ...
            if (GlobalUtil::_TruncateMethod &&
                GlobalUtil::_FeatureCountThreshold > 0 &&
    ...
    for (int i = 0, idx = 0; i < _octave_num; i++) {
        for (int j = 0; j < param._dog_level_num; j++, idx++) {
            ...
            glGetBufferParameteriv(GL_PIXEL_PACK_BUFFER_ARB, GL_BUFFER_SIZE,
                                   &bsize);
            esize =
                ftex->GetImgHeight() * ftex->GetImgWidth() * sizeof(float) * 4;

            // increase size when necessary
            if (bsize < esize) {
                glBufferData(GL_PIXEL_PACK_BUFFER_ARB, esize * 3 / 2, NULL,
                             GL_STATIC_DRAW_ARB);
                glGetBufferParameteriv(GL_PIXEL_PACK_BUFFER_ARB, GL_BUFFER_SIZE,
                                       &bsize);
            }
            ...
            if (bsize >= esize)
                glReadPixels(0, 0, ftex->GetImgWidth(), ftex->GetImgHeight(),
                             GL_RGBA, GL_FLOAT, 0);
            else
                glBufferData(GL_PIXEL_PACK_BUFFER_ARB, 0, NULL,
                             GL_STATIC_DRAW_ARB);
...
void PyramidNaive::GetFeatureOrientations() {
    ...
    for (int i = 0; i < _octave_num; i++) {
        ...
        for (int j = 0; j < param._dog_level_num;
             j++, ftex++, otex++, count++, gtex++, stex++) {
            if (*count <= 0)
                continue;
            ...
            if (_orientationTex)
                otex->AttachToFBO(1);
            if (!_existing_keypoints && (GlobalUtil::_SubpixelLocalization ||
                                         GlobalUtil::_KeepExtremumSign)) {
                glActiveTexture(GL_TEXTURE2);
                stex->BindTex();
                sid = *stex;
            }
            ShaderMan::UseShaderOrientation(
                gtex->GetTexID(), gtex->GetImgWidth(), gtex->GetImgHeight(),
...
void PyramidNaive::GenerateFeatureListCPU() {
    ...
    for (int i = 0; i < _octave_num; i++) {
        for (int j = 0; j < param._dog_level_num; j++, idx++) {
            ...
            // tex->AttachToFBO(0);
            // tex->FitTexViewPort();
            // glReadPixels(0, 0, tex->GetTexWidth(), tex->GetTexHeight(),
            // GL_RED, GL_FLOAT, mem);
            //
            // make a list of
            list.resize(0);
            ...
            for (int k = 0; k < tex->GetTexHeight(); k++) {
                for (int m = 0; m < tex->GetTexWidth(); m++, p++) {
                    if (*p == 0)
                        continue;
                    if (m == 0 || k == 0 || k >= tex->GetImgHeight() - 1 ||
                        m >= tex->GetImgWidth() - 1)
                        continue;
            ...
            ftex->BindTex();
            ftex->AttachToFBO(0);
            //		glTexImage2D(GlobalUtil::_texTarget, 0,
            // GlobalUtil::_iTexFormat, fw, fh, 0, GL_BGRA, GL_FLOAT, &list[0]);
            glTexSubImage2D(GlobalUtil::_texTarget, 0, 0, 0, fw, fh, GL_RGBA,
                            GL_FLOAT, &list[0]);
            //
        }
    }
...
void PyramidGL::ReshapeFeatureListCPU() {
    ...
    glBufferData(GL_PIXEL_UNPACK_BUFFER_ARB, szmax * 8 * sizeof(float), NULL,
                 GL_STREAM_DRAW);
#endif
    ...
    _keypoint_buffer.resize(0);
    float os = _octave_min >= 0 ? float(1 << _octave_min)
                                : 1.0f / (1 << (-_octave_min));
    ...
    for (i = 0; i < n; i++) {
        ...
        glReadPixels(0, 0, _featureTex[i].GetImgWidth(),
        ...
        if (GlobalUtil::_OrientationPack2 == 0) {
            ...
            glReadPixels(0, 0, _orientationTex[i].GetImgWidth(),
                         _orientationTex[i].GetImgHeight(), GL_RGBA, GL_FLOAT,
                         buffer2);
            // make the feature list
            for (int j = 0; j < _levelFeatureNum[i];
                 j++, src += 4, orientation += 4) {
        ...
        glTexSubImage2D(GlobalUtil::_texTarget, 0, 0, 0, nfw, nfh, GL_RGBA,
                        GL_FLOAT, buffer3);
        ...
        if (fcount > 0) {
            ...
            for (int k = 0; k < fcount; k++, ds += 4, fs += 4) {
                ...
                ds[3] =
                    (float)fmod(twopi - fs[2], twopi); // orientation, mirrored
                ds[2] = oss * fs[3];                   // scale
            }
        }
#endif
...
void PyramidNaive::GetSimplifiedOrientation() {
    ...
    for (int i = 0; i < _octave_num; i++) {
        GLTexImage *gtex =
            GetLevelTexture(i + _octave_min, 2 + param._level_min);
...
#else
    // descriptor normalization runs on cpu for OpenGL implementations
    for (int k = 0; k < num; k++, pd += 128) {
        int v;
        float *ppd, sq = 0;
        // int v;
...
inline void PyramidGL::InterlaceDescriptorF2(int w, int h, float *buf,
                                             float *pd, int step) {
    /*
    if(GlobalUtil::_DescriptorPPR == 8)
    {
            ...
            for(int u = 0; u < h ; u++, pd+=dstep)
            {
                    ...
                    for(v= 0; v < w; v++)
                    {
                            ...
                            {
                                    *ppd++ = *pp1++;*ppd++ = *pp1++;*ppd++ =
    *pp1++;*ppd++ = *pp1++; *ppd++ = *pp2++;*ppd++ = *pp2++;*ppd++ =
    *pp2++;*ppd++ = *pp2++;
                            }
                            ppd += 64;
                    }
...
void PyramidGL::GetFeatureDescriptors() {
    ...
    for (i = 0, idx = 0, ftex = _featureTex; i < _octave_num; i++) {
        ...
        for (j = 0; j < param._dog_level_num;
             j++, ftex++, idx++, gtex++, otex++) {
            if (_levelFeatureNum[idx] == 0)
                continue;

            sigma = IsUsingRectDescription()
                        ? 0
                        : param.GetLevelSigma(j + param._level_min + 1);
            ...
            if (w > _descriptorTex[0].GetTexWidth() ||
                h > _descriptorTex[0].GetTexHeight()) {
                for (k = 0; k < ndf; k++)
            ...
            ShaderMan::UseShaderDescriptor(
                gtex->GetTexID(), otex->GetTexID(), w, ftex->GetImgWidth(),
            ...
            for (k = 0; k < ndf; k++, pbuf += step) {
                ...
                if (GlobalUtil::_IsNvidia || w * h <= 16384) // were
                {
                    ...
                    for (int kk = 0; kk < h; kk += hstep)
                        glReadPixels(0, kk, w, min(hstep, h - kk), GL_RGBA,
                                     GL_FLOAT, pbuf + w * kk * 4);
                }
            }
            ...
            InterlaceDescriptorF2(w / block_width, h / block_height,
                                  &read_buffer[0], pd, step);
    ...
    // finally, put the descriptor back to their original order for existing
    // keypoint list.
    if (_keypoint_index.size() > 0) {
        for (i = 0; i < _featureNum; ++i) {
            int index = _keypoint_index[i];
            memcpy(&_descriptor_buffer[index * 128],
                   &descriptor_buffer2[i * 128], 128 * sizeof(float));
        }
    }
...
void PyramidGL::DownloadKeypoints() {
    ...
    // use a different keypoint buffer when processing with an exisint features
    // list without orientation information.
    if (_keypoint_index.size() > 0) {
        ...
        for (int j = 0; j < param._dog_level_num; j++, idx++, ftex++) {
            ...
            if (_levelFeatureNum[idx] > 0) {
                ...
                for (int k = 0; k < _levelFeatureNum[idx]; k++, ps += 4) {
                    ...
                    sigma = os * ps[3];
                    ps[3] = (float)fmod(twopi - ps[2],
                                        twopi); // orientation, mirrored
                    ps[2] = sigma;              // scale
                }
    ...
    if (_keypoint_index.size() > 0) {
        for (int i = 0; i < _featureNum; ++i) {
            int index = _keypoint_index[i];
            memcpy(&_keypoint_buffer[index * 4], &keypoint_buffer2[i * 4],
                   4 * sizeof(float));
        }
    }
}
...
void PyramidGL::GenerateFeatureListTex() {
    ...
    float octave_sigma = _octave_min >= 0 ? float(1 << _octave_min)
                                          : 1.0f / (1 << (-_octave_min));
    float offset = GlobalUtil::_LoweOrigin ? 0 : 0.5f;
    if (_down_sample_factor > 0)
    ...
    for (int i = 0; i < _octave_num; i++, octave_sigma *= 2.0f) {
        for (int j = 0; j < param._dog_level_num; j++, idx++) {
            list.resize(0);
            float level_sigma =
                param.GetLevelSigma(j + param._level_min + 1) * octave_sigma;
            ...
            for (int k = 0; k < _featureNum; k++) {
                ...
                if ((sigmak >= sigma_min && sigmak < sigma_max) ||
                    ...
                    (sigmak > sigma_max && j == param._dog_level_num - 1 &&
                     (i == _octave_num - 1 ||
                      GlobalUtil::_KeyPointListForceLevel0))) {
...
PyramidPacked::~PyramidPacked() { DestroyPyramidData(); }
...
void PyramidPacked::ComputeGradient() {
    ...
    GLenum buffers[4] = {GL_COLOR_ATTACHMENT0_EXT, GL_COLOR_ATTACHMENT1_EXT,
                         GL_COLOR_ATTACHMENT2_EXT, GL_COLOR_ATTACHMENT3_EXT};
...
void PyramidPacked::DetectKeypointsEX() {
    ...
    for (i = _octave_min; i < _octave_min + _octave_num; i++) {
        ...
        gus->FitTexViewPort();
        // compute the gradient
        for (j = param._level_min + 1; j <= param._level_max;
             j++, gus++, dog++, grd++, rot++) {
            // gradient, dog, orientation
            glActiveTexture(GL_TEXTURE0);
            gus->BindTex();
...
void PyramidPacked::GenerateFeatureList(int i, int j) {
    ...
    // this part might have problems on several GPUS
    // because the output of one pass is the input of the next pass
    // may require glFinish to make it right, but too much glFinish makes it
    // slow
    for (int k = 0; k < hist_level_num - i - 1 - hist_skip_gpu; k++, htex--) {
        htex->AttachToFBO(0);
        htex->FitTexViewPort();
        ...
        for (int j = 0; j < param._dog_level_num; j++, idx++) {
            ...
            for (int k = 0; k < tex->GetDrawHeight(); k++) {
                ...
                for (int m = 0; m < tex->GetDrawWidth(); m++, p += 4) {
                    //	if(m ==0 || k ==0 || k == tex->GetDrawHeight() -1 || m
                    //== tex->GetDrawWidth() -1) continue; 	if(*p == 0)
                    // continue;
                    ...
                    if (xx == 0 || yy == 0)
                        continue;
                    if (xx >= tex->GetImgWidth() - 1 ||
                        yy >= tex->GetImgHeight() - 1)
                        continue;
                    list.push_back(xx + 0.5f + p[1]);
                    list.push_back(yy + 0.5f + p[2]);
                    list.push_back(GlobalUtil::_KeepExtremumSign && p[0] < 0
                                       ? -1.0f
                                       : 1.0f);
                    list.push_back(p[3]);
                    fcount++;
...
void PyramidPacked::GetSimplifiedOrientation() {
    ...
    for (int i = 0; i < _octave_num; i++) {
        ...
        for (int j = 0; j < param._dog_level_num; j++, ftex++, otex++, idx++) {
            ...
            ShaderMan::UseShaderSimpleOrientation(otex->GetTexID(), sigma,
                                                  sigma_step);
            ftex->DrawQuad();
        }
    }
...
void PyramidPacked::InitPyramid(int w, int h, int ds) {
    ...
    while (GlobalUtil::_MemCapGPU > 0 && GlobalUtil::_FitMemoryCap &&
           (wp > _pyramid_width || hp > _pyramid_height) &&
           max(max(wp, hp), max(_pyramid_width, _pyramid_height)) >
               1024 * sqrt(GlobalUtil::_MemCapGPU / 96.0)) {
        _octave_min++;
        wp >>= 1;
        hp >>= 1;
        toobig = 2;
