...
//		            class ProgramCL			A simple wrapper
// of Cg programs
//
...
ProgramCL::ProgramCL(const char *name, const char *code, cl_context context,
                     cl_device_id device)
    : _valid(1) {
    const char *src[1] = {code};
    cl_int status;
...
bool ProgramBagCL::InitializeContext() {
    ...
    ///////////////////////////////
    status =
        clGetDeviceIDs(_platform, CL_DEVICE_TYPE_GPU, 0, NULL, &num_device);
    if (status != CL_SUCCESS || num_device == 0)
        return false;

    // Create the device list
    cl_device_id *devices = new cl_device_id[num_device];
    status = clGetDeviceIDs(_platform, CL_DEVICE_TYPE_GPU, num_device, devices,
                            NULL);
    ...
    if (GlobalUtil::_verbose) {
        ...
        clGetDeviceInfo(_device, CL_DEVICE_GLOBAL_MEM_CACHE_TYPE,
                        sizeof(is_gcache), &is_gcache, NULL);
        if (is_gcache == CL_NONE)
            std::cout << "No cache for global memory\n";
        // else if(is_gcache == CL_READ_ONLY_CACHE) std::cout << "Read only
        // cache for global memory\n"; else std::cout << "Read/Write cache for
        // global memory\n";
    }
...
void ProgramBagCL::FinishCL() { clFinish(_queue); }

void ProgramBagCL::LoadFixedShaders() {

    s_gray = new ProgramCL(
        "gray",
        "__kernel void gray(__read_only  image2d_t input, __write_only "
        "image2d_t output) {\n"
    ...
    s_sampling = new ProgramCL(
        "sampling",
        "__kernel void sampling(__read_only  image2d_t input, __write_only "
        "image2d_t output,\n"
    ...
    s_sampling_k = new ProgramCL(
        "sampling_k",
        "__kernel void sampling_k(__read_only  image2d_t input, __write_only "
        "image2d_t output, "
        "                   int width, int height,\n"
    ...
    s_zero_pass = new ProgramCL(
        "zero_pass",
        "__kernel void zero_pass(__write_only image2d_t output){\n"
        "int2 coord = (int2)(get_global_id(0),  get_global_id(1));\n"
        "write_imagef(output, coord, (float4)(0.0));}",
        _context, _device);

    s_packup = new ProgramCL(
        "packup",
        "__kernel void packup(__global float* input, __write_only image2d_t "
        "output,\n"
        ...
        "write_imagef(output, coord, (float4) (v1, v2, v3, v4));}",
        _context, _device);

    s_dog_pass = new ProgramCL(
        "dog_pass",
        "__kernel void dog_pass(__read_only image2d_t tex,  __read_only "
        "image2d_t texp,\n"
    ...
    s_grad_pass = new ProgramCL(
        "grad_pass",
        "__kernel void grad_pass(__read_only image2d_t tex,  __read_only "
        "image2d_t texp,\n"
        "                   __write_only image2d_t dog, int width, int "
        "height,\n"
        "                    __write_only image2d_t grad, __write_only "
        "image2d_t rot) {\n"
        "sampler_t sampler = CLK_NORMALIZED_COORDS_FALSE |\n"
        "                    CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST;\n"
        "int x = get_global_id(0), y =  get_global_id(1); \n"
        ...
        "write_imagef(dog, coord, cc - cp); \n"
        "float4 dx = (float4)(cc.y - cl.x,  cr.x - cc.x, cc.w - cl.y, cr.y - "
        "cc.z);\n"
    ...
    s_grad_pass2 = new ProgramCL(
        "grad_pass2",
        "#define BLOCK_DIMX 32\n"
        "#define BLOCK_DIMY 14\n"
        "#define BLOCK_SIZE (BLOCK_DIMX * BLOCK_DIMY)\n"
        "__kernel void grad_pass2(__read_only image2d_t tex,  __read_only "
        "image2d_t texp,\n"
        "                   __write_only image2d_t dog, int width, int "
        "height,\n"
        "                   __write_only image2d_t grd, __write_only image2d_t "
        "rot){\n" //,  __local float* block) {\n"
        ...
        "if( get_local_id(0) == 0 || get_local_id(0) == BLOCK_DIMX - 1) "
        "return;\n"
        "if( get_local_id(1) == 0 || get_local_id(1) == BLOCK_DIMY - 1) "
        "return;\n"
        ...
        "float4 dy = (float4)(  cc.z - block[idx - BLOCK_DIMX + 2 * "
        "BLOCK_SIZE], \n"
        "                       cc.w - block[idx - BLOCK_DIMX + 3 * "
        "BLOCK_SIZE],"
...
void ProgramBagCL::SelectInitialSmoothingFilter(int octave_min,
                                                SiftParam &param) {
...
ProgramCL *ProgramBagCL::CreateFilterH(float kernel[], int width) {
    ...
    out << "const sampler_t sampler = CLK_NORMALIZED_COORDS_FALSE | "
           "CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST;"
    ...
    for (int i = 0; i < npixel; i++) {
        ...
        if (GlobalUtil::_PreciseBorder)
            out << "if(coord.x < 0) pc = pc.xxzz; else if (coord.x > width_) "
                   "pc = pc.yyww; \n";
        ...
        if (weight[1] == 0.0) {
            out << "result += (float4)(" << weight[2] << "," << weight[0] << ","
                << weight[2] << "," << weight[0] << ") * pc.yxwz;\n";
        } else {
            out << "result += (float4)(" << weight[1] << ", " << weight[0]
                << ", " << weight[1] << ", " << weight[0] << ") * pc.xxzz;\n";
            out << "result += (float4)(" << weight[2] << ", " << weight[1]
                << ", " << weight[2] << ", " << weight[1] << ") * pc.yyww;\n";
        }
    }
...
ProgramCL *ProgramBagCL::CreateFilterV(float kernel[], int width) {
    ...
    for (int i = 0; i < npixel; i++) {
        ...
        out << "pc= read_imagef(input, sampler, coord);\n";
        if (GlobalUtil::_PreciseBorder)
            out << "if(coord.y < 0) pc = pc.xyxy; else if (coord.y > height_) "
                   "pc = pc.zwzw; \n";
        // for each sub-pixel j  in center, the weight of sub-pixel k
        int xw = (i - nhpixel) * 2;
        for (int j = 0; j < 3; j++) {
        ...
        if (weight[1] == 0.0) {
            out << "result += (float4)(" << weight[2] << "," << weight[2] << ","
                << weight[0] << "," << weight[0] << ") * pc.zwxy;\n";
        } else {
            out << "result += (float4)(" << weight[1] << ", " << weight[1]
                << ", " << weight[0] << ", " << weight[0] << ") * pc.xyxy;\n";
            out << "result += (float4)(" << weight[2] << ", " << weight[2]
                << ", " << weight[1] << ", " << weight[1] << ") * pc.zwzw;\n";
        }
    }
    out << "write_imagef(output, (int2)(x, y), result); }\n" << '\0';
...
void ProgramBagCL::FilterImage(FilterCL *filter, CLTexImage *dst,
                               CLTexImage *src, CLTexImage *tmp) {
    ...
    size_t gsz[2] = {(w + dim0 - 1) / dim0 * dim0,
                     (h + dim1 - 1) / dim1 * dim1},
           lsz[2] = {dim0, dim1};
    ...
    status = clEnqueueNDRangeKernel(_queue, kernelh, 2, NULL, gsz, lsz, 0, NULL,
                                    NULL);
    ...
    size_t gsz2[2] = {(w + dim1 - 1) / dim1 * dim1,
                      (h + dim0 - 1) / dim0 * dim0},
           lsz2[2] = {dim1, dim0};
    status = clEnqueueNDRangeKernel(_queue, kernelv, 2, NULL, gsz2, lsz2, 0,
                                    NULL, NULL);
    CheckErrorCL(status, "ProgramBagCL::FilterImageV");
    // clReleaseEvent(event);
}

void ProgramBagCL::SampleImageU(CLTexImage *dst, CLTexImage *src,
                                int log_scale) {
    ...
    size_t gsz[2] = {(w + dim0 - 1) / dim0 * dim0,
                     (h + dim1 - 1) / dim1 * dim1},
           lsz[2] = {dim0, dim1};
    cl_int status = clEnqueueNDRangeKernel(_queue, kernel, 2, NULL, gsz, lsz, 0,
                                           NULL, NULL);
    CheckErrorCL(status, "ProgramBagCL::SampleImageU");
}

void ProgramBagCL::SampleImageD(CLTexImage *dst, CLTexImage *src,
                                int log_scale) {
    cl_kernel kernel;
    cl_int w = dst->GetImgWidth(), h = dst->GetImgHeight();
    if (log_scale == 1) {
...
void ProgramBagCL::ComputeDOG(CLTexImage *tex, CLTexImage *texp,
                              CLTexImage *dog, CLTexImage *grad,
                              CLTexImage *rot) {
...
void ProgramBagCL::ComputeKEY(CLTexImage *dog, CLTexImage *key, float Tdog,
                              float Tedge) {
...
void ProgramBagCL::UnpackImageKEY(CLTexImage *src, CLTexImage *dog,
                                  CLTexImage *dst) {
...
void ProgramBagCL::LoadKeypointShader() {
    ...
    out << "__kernel void keypoint(__read_only image2d_t tex, __read_only "
           "image2d_t texU,\n"
           "           __read_only image2d_t texD, __write_only image2d_t "
           "texK,\n"
    ...
    for (i = 0; i < 4; ++i)
        out << "   if(cc.s" << i
            ...
            << i << "), max(v1[" << i << "], v2[" << i << "]))))key4.s" << i
            << " = 1.0;\n"
               "	}else if(cc.s"
            << i
            << " < -THRESHOLD0){ \n"
               "           if(all(isless((float4)(cc.s"
            << i << "), min(v1[" << i << "], v2[" << i << "]))))key4.s" << i
            << " = -1.0;\n"
               "   }";
    ...
    for (i = 0; i < 4; ++i)
        out << "	if(key4.s" << i
            ...
            << i
            << "] = 0.25*(D4.x + D4.y);\n"
               "		float fxx_plus_fyy = fxx["
            << i << "] + fyy[" << i
            << "];\n"
               "		float score_up = fxx_plus_fyy*fxx_plus_fyy; \n"
               "		float score_down = (fxx["
            << i << "]*fyy[" << i << "] - fxy[" << i << "]*fxy[" << i
            << "]);\n"
    ...
    if (GlobalUtil::_SubpixelLocalization) {
        out << "	float4 offset = (float4)(0); \n";
        for (i = 1; i < 4; ++i)
            out << "	if(key4.s" << i
                << " != 0) \n"
                   "	{\n"
                   "		cu.s0 = cu.s"
                << i << ";	cd.s0 = cd.s" << i << ";	cc.s0 = cc.s"
                << i
                << ";	\n"
                   "		v4[0] = v4["
                << i << "];	v5[0] = v5[" << i
        ...
        out << "	float fs = 0.5*( cu.s0 - cd.s0 );		"
               ...
               "	float fxs = 0.25 * (v4[0].y + v5[0].x - v4[0].x - "
               "v5[0].y);\n"
               "	float fys = 0.25 * (v4[0].w + v5[0].z - v4[0].z - "
               "v5[0].w);\n"
               "	float4 A0, A1, A2 ;			\n"
               "	A0 = (float4)(fxx[0], fxy[0], fxs, -fx[0]);	\n"
               "	A1 = (float4)(fxy[0], fyy[0], fys, -fy[0]);	\n"
               "	A2 = (float4)(fxs, fys, fss, -fs);	\n"
               "	float4 x3 = fabs((float4)(fxx[0], fxy[0], fxs, "
               "0));		\n"
               ...
               "		{					"
               "						\n"
               "			float4 TEMP = A1; A1 = A0; A0 = "
               "TEMP;	\n"
               "		}else if( x3.z == maxa )		"
               "			\n"
               "		{					"
               "						\n"
               "			float4 TEMP = A2; A2 = A0; A0 = "
               "TEMP;	\n"
               "		}					"
               "						\n"
               "		A0 /= A0.x;				"
               ...
               "offset.z*A1.z;			    \n"
               "				offset.x = A0.w - "
               "offset.z*A0.z - offset.y*A0.y;	\n"
               "				if(fabs(cc.s0 + "
               "0.5*dot((float4)(fx[0], fy[0], fs, 0), offset ))<=THRESHOLD1\n"
               "                   || any( isgreater(fabs(offset), "
               "(float4)(1.0)))) key4 = (float4)(0.0);\n"
               "			}\n"
               "		}\n"
               "	}\n"
            << "\n"
...
void ProgramBagCL::LoadDisplayShaders() {
    ...
    // pc.r : pc.g):(ff.x?pc.b:pc.a); gl_FragColor = vec4(vec3(v), 1.0);}");
    s_unpack = new ProgramCL(
        "main",
        "__kernel void main(__read_only  image2d_t input, __write_only "
        "image2d_t output,\n"
        "                   int width, int height) {\n"
        "sampler_t sampler = CLK_NORMALIZED_COORDS_FALSE | "
        "CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST;\n"
    ...
    s_unpack_key = new ProgramCL(
        ...
        "float4 result;\n"
        "if(k0 != 0){\n"
        "   //result = kk.x > 0 ? ((float4)(1.0, 0, 0, 1.0)) : ((float4) (0.0, "
        "1.0, 0.0, 1.0)); \n"
        "   result = kk.x < 0 ? ((float4)(0, 1.0, 0, 1.0)) : ((float4) (1.0, "
        "0.0,  0.0, 1.0)); \n"
        "}else{"
...
void ProgramBagCL::SetSimpleOrientationInput(int oTex, float sigma,
                                             float sigma_step) {}

void ProgramBagCL::SetFeatureOrientationParam(int gtex, int width, int height,
                                              float sigma, int otex,
                                              float step) {}

void ProgramBagCL::SetFeatureDescirptorParam(int gtex, int otex, float dwidth,
...
void ProgramBagCLN::LoadFixedShaders() {
    s_sampling = new ProgramCL(
        ...
        "int2 coord = (int2)(get_global_id(0), get_global_id(1)); \n"
        "if( coord.x >= width || coord.y >= height) return;\n"
        "write_imagef(output, coord, read_imagef(input, sampler, coord << 1)); "
        "}",
        _context, _device);
    ...
    s_sampling_k = new ProgramCL(
        ...
        "write_imagef(output, (int2) (x, y), v1); }",
        _context, _device);
    ...
    s_sampling_u = new ProgramCL(
        ...
        "float xa = x * step,       ya = y *step; \n"
        "float v1 = read_imagef(input, sampler, (float2) (xa, ya)).x; \n"
        "write_imagef(output, (int2) (x, y), (float4)(v1)); }",
        _context, _device);
...
ProgramCL *ProgramBagCLN::CreateFilterH(float kernel[], int width) {
    ...
    out << "#define KERNEL_WIDTH " << width << "\n"
        ...
        << "\n"
           ...
           "    {\n"
           "        int fetch_index = min(x + j * BLOCK_WIDTH - "
           "KERNEL_HALF_WIDTH, width_);\n"
           "        data[get_local_id(0) + j * BLOCK_WIDTH] = "
           "read_imagef(input, sampler, (int2)(fetch_index, y)).x;\n"
           "    }\n"
           "}\n"
           "barrier(CLK_LOCAL_MEM_FENCE); \n"
           "if( x > width_ || y > height_) return; \n"
...
ProgramCL *ProgramBagCLN::CreateFilterV(float kernel[], int width) {
    ...
    out << "#define KERNEL_WIDTH " << width << "\n"
        ...
        << "\n"
           ...
           "    {\n"
           "        int fetch_index = min(y + j * BLOCK_WIDTH - "
           "KERNEL_HALF_WIDTH, height_);\n"
           "        data[get_local_id(1) + j * BLOCK_WIDTH ] = "
           "read_imagef(input, sampler, (int2)(x, fetch_index)).x;\n"
           "    }\n"
           "}\n"
           "barrier(CLK_LOCAL_MEM_FENCE); \n"
           "if( x > width_ || y > height_) return; \n"
...
void ProgramBagCLN::FilterImage(FilterCL *filter, CLTexImage *dst,
                                CLTexImage *src, CLTexImage *tmp) {
    cl_kernel kernelh = filter->s_shader_h->_kernel;
    cl_kernel kernelv = filter->s_shader_v->_kernel;
    //////////////////////////////////////////////////////////////////
    ...
    size_t gsz1[2] = {(w + dim00 - 1) / dim00 * dim00,
                      (h + dim01 - 1) / dim01 * dim01},
           lsz1[2] = {dim00, dim01};
    status = clEnqueueNDRangeKernel(_queue, kernelh, 2, NULL, gsz1, lsz1, 0,
                                    NULL, NULL);
    ...
    size_t gsz2[2] = {(w + dim10 - 1) / dim10 * dim10,
                      (h + dim11 - 1) / dim11 * dim11},
           lsz2[2] = {dim10, dim11};
    status = clEnqueueNDRangeKernel(_queue, kernelv, 2, NULL, gsz2, lsz2, 0,
                                    NULL, NULL);
...
void ProgramBagCLN::SampleImageD(CLTexImage *dst, CLTexImage *src,
                                 int log_scale) {
    cl_kernel kernel;
    cl_int w = dst->GetImgWidth(), h = dst->GetImgHeight();
