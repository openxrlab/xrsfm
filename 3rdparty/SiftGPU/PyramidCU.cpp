...
void PyramidCU::InitPyramid(int w, int h, int ds) {
    ...
    while (GlobalUtil::_MemCapGPU > 0 && GlobalUtil::_FitMemoryCap &&
           ...
           max(max(wp, hp), max(_pyramid_width, _pyramid_height)) >
               1024 * sqrt(GlobalUtil::_MemCapGPU / 110.0)) {
        _octave_min++;
        wp >>= 1;
        hp >>= 1;
        toobig = 2;
    ...
    if (toobig && GlobalUtil::_verbose && _octave_min > 0) {
        std::cout
            << (toobig == 2
                    ...
                    : "[**SKIP OCTAVES**]:\tReaching the dimension "
                      "limit(-maxd)!\n");
    }
    // ResizePyramid(wp, hp);
...
void PyramidCU::FitPyramid(int w, int h) {
    ...
    int _octave_num_max =
        max(1, (int)floor(log(double(min(w, h))) / log(2.0)) - 3);
    ...
    for (int i = 0, idx = 0; i < _octave_num; i++) {
        ...
        for (int j = 0; j < param._dog_level_num; j++, ftex++, idx++, got++) {
            ...
            ProgramCU::ComputeDescriptor(ftex, got, _descriptorTex,
                                         IsUsingRectDescription()); // process
        ...
        for (int j = 0; j < param._dog_level_num; j++, idx++) {
            ...
            for (int k = 0; k < _featureNum; k++) {
                ...
                if ((sigmak >= sigma_min && sigmak < sigma_max) ||
                    ...
                    (sigmak > sigma_max && i == _octave_num - 1 &&
...
void PyramidCU::GenerateFeatureDisplayVBO() {
    ...
    for (int i = 0; i < nvbo; i++) {
        ...
        CuTexImage texPBO1(_levelFeatureNum[i] * 10, 1, 4,
                           _featureDisplayVBO[i]);
...
void PyramidCU::GenerateFeatureList(int i, int j, int reduction_count,
                                    vector<int> &hbuffer) {
...
void PyramidCU::GenerateFeatureList() {
    ...
    FOR_EACH_OCTAVE(i, reverse) {
        ...
        FOR_EACH_LEVEL(j, reverse) {
            ...
            // of level features will not match the absolute number of features.
...
GLTexImage *PyramidCU::ConvertTexCU2GL(CuTexImage *tex, int dataName) {
    ...
    case DATA_DOG: {
        CuTexImage texPBO(tex->GetImgWidth(), tex->GetImgHeight(), 1,
                          _bufferPBO);
    ...
    case DATA_KEYPOINT: {
        ...
        format = GL_RGBA;
        CuTexImage texPBO(tex->GetImgWidth(), tex->GetImgHeight(), 4,
                          _bufferPBO);
        if (texPBO._cuData == 0 || tex->_cuData == NULL)
            convert_done = 0;
        else
    ...
    if (convert_done) {
        _bufferTEX->InitTexture(
            max(_bufferTEX->GetTexWidth(), tex->GetImgWidth()),
            max(_bufferTEX->GetTexHeight(), tex->GetImgHeight()));
        _bufferTEX->CopyFromPBO(_bufferPBO, tex->GetImgWidth(),
                                tex->GetImgHeight(), format);
    } else {
...
GLTexImage *PyramidCU::GetLevelTexture(int octave, int level, int dataName) {
    ...
    // CuTexImage* gus = GetBaseLevel(octave, DATA_GAUSSIAN) + (level -
    // param._level_min);
...
void PyramidCU::ConvertInputToCU(GLTexInput *input) {
    ...
    if (input->_pixel_data) {
        ...
        if (_bufferPBO == 0)
            glGenBuffers(1, &_bufferPBO);
        if (input->_rgb_converted &&
            input->CopyToPBO(_bufferPBO, ws, hs, GL_LUMINANCE)) {
            _inputTex->InitTexture(ws, hs, 1);
            _inputTex->CopyFromPBO(ws, hs, _bufferPBO);
        } else if (input->CopyToPBO(_bufferPBO, ws, hs)) {
            CuTexImage texPBO(ws, hs, 4, _bufferPBO);
            _inputTex->InitTexture(ws, hs, 1);
            ProgramCU::ReduceToSingleChannel(_inputTex, &texPBO,
                                             !input->_rgb_converted);
        } else {
        ...
        if (i == _octave_min) {
            ...
            if (i == 0) {
                ProgramCU::FilterImage(tex, _inputTex, buf,
                                       param.GetInitialSmoothSigma(
                                           _octave_min + _down_sample_factor));
            } else {
                if (i < 0)
            ...
            ProgramCU::SampleImageD(tex, GetBaseLevel(i - 1) + param._level_ds -
                                             param._level_min);
...
void PyramidCU::DetectKeypointsEX() {
    ...
    for (i = _octave_min; i < _octave_min + _octave_num; i++) {
        ...
        for (j = param._level_min + 2; j < param._level_max;
            ...
            // output, key
            ProgramCU::ComputeKEY(dog, key, param._dog_threshold,
