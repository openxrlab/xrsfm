...
ProgramCG::ProgramCG() { _programID = NULL; }
...
ProgramCG::ProgramCG(const char *code, const char **cg_compile_args,
                     CGprofile profile) {
    _valid = 0;
    _profile = profile;
    GLint epos;
    const char *ati_args[] = {"-po", "ATI_draw_buffers", 0};
    const char *fp40_args[] = {"-ifcvt",
                               "none",
                               "-unroll",
                               "all",
                               GlobalUtil::_UseFastMath ? "-fastmath" : 0,
                               0};
    if (cg_compile_args == NULL)
        cg_compile_args = GlobalUtil::_IsNvidia
                              ? (GlobalUtil::_SupportFP40 ? fp40_args : NULL)
                              : ati_args;
    ...
    if (_programID) {
        ...
        if (epos >= 0) {
            std::cout << cgGetProgramString(_programID, CG_COMPILED_PROGRAM)
                      << endl;
...
void ProgramCG::DestroyContext() { cgDestroyContext(_Context); }

ShaderBagCG::ShaderBagCG() { ProgramCG::InitContext(); }
...
void ShaderBagCG::LoadFixedShaders() {
    //	s_debug = new ProgramCG( "void main(float4 TexCoord0:TEXCOORD0, out
    // float4 FragColor:COLOR0," 		"uniform samplerRECT tex){
    // gl_FragColor.rg = gl_TexCoord[0].st;}");

    s_gray = new ProgramCG(
        "void main(float4 TexCoord0 : TEXCOORD0, out float4 FragColor : "
        "COLOR0, uniform samplerRECT tex){\n"
        "float intensity = dot(float3(0.299, 0.587, 0.114), "
        "texRECT(tex,TexCoord0.xy ).rgb);\n"
        "FragColor= float4(intensity, intensity, intensity, 1.0);}");

    s_sampling =
        new ProgramCG("void main(float4 TexCoord0 : TEXCOORD0, out float4 "
                      "FragColor : COLOR0, uniform samplerRECT tex){\n"
                      "float4 cc = texRECT(tex, TexCoord0.xy);	FragColor = "
                      "float4(cc.rg, 0.0, 0.0);	}");

    s_zero_pass = new ProgramCG(
        "void main(out float4 FragColor : COLOR0){FragColor = 0;}");
    ...
    s_grad_pass = new ProgramCG(
        ...
        "float4 TexRC : TEXCOORD2, float4 TexCD : TEXCOORD3, float4 TexCU : "
        "TEXCOORD4,\n"
...
void ShaderBagCG::LoadDisplayShaders() {
    s_copy_key =
        new ProgramCG("void main(float4 TexCoord0 : TEXCOORD0, out float4 "
                      "FragColor : COLOR0, uniform samplerRECT tex){\n"
                      "FragColor.rg= texRECT(tex, TexCoord0.xy).rg; "
                      "FragColor.ba = float2(0,1);	}");
    ...
    s_vertex_list = program = new ProgramCG(
        ...
        "	dxy.x = type < 0.1 ? 0 : ((type <0.5 || type > 0.9)? size : "
        "-size);\n"
        "	dxy.y = type < 0.2 ? 0 : ((type < 0.3 || type > 0.7 )? -size "
        ":size); \n"
        "	sincos(cc.b, s, c);\n"
    ...
    s_display_keys =
        new ProgramCG("void main(float4 TexCoord0 : TEXCOORD0, out float4 "
                      ...
                      "else {if (cc.r ==0.5) FragColor = "
                      "float4(0.0,1.0,0.0,1.0);	else discard;}}");
}
...
int ShaderBagCG::LoadKeypointShaderMR(float threshold, float edge_threshold) {
    ...
    out << "void main (\n"
           ...
           "uniform samplerRECT tex, uniform samplerRECT texU, uniform "
           "samplerRECT texD)\n"
           "{\n"
           "	float4 v1, v2, gg;\n"
        ...
        << "	if( cc.g > THRESHOLD0 && all(cc.gggg > max(v1, v2)))\n"
           ...
           "		if(cc.g < v3.x || any(cc.gggg<v4.xyzw || "
           "cc.gggg<v6.xyzw))return; \n"
           ...
           "		if(cc.g < v3.y || any(cc.gggg<v5.xyzw || "
           "cc.gggg<v6.xyzw))return; \n"
           "		dog = 1.0; \n"
           "	}\n"
        // the minimum case
        ...
        << "				dxys.z = A2.w /A2.z;		"
           ...
           "	}\n"
           "  }while(key_moved);\n"
        << "  bool test1 = (abs(cc.g + 0.5*dot(float3(fx, fy, fs), dxys ))> "
           "THRESHOLD1) ;\n"
        ...
        << "  FragData1 = test1 && test2 && all( abs(dxys) < 1)? float4( dog, "
           "dxys.xy+offsets, dxys.z) : float4(0, 0, 0, 0); \n"
           "}\n"
        << '\0';
...
void ShaderBagCG::LoadKeypointShader(float threshold, float edge_threshold) {
    ...
    out << "void main (\n"
        ...
        << "	float dog = 0.0; \n"
           "	FragData1 = float4(0, 0, 0, 0); \n"
           "	dog = cc.g > THRESHOLD0 && all(cc.gggg > max(v1, v2))?1.0: "
           "0.0;\n"
           "	dog = cc.g < -THRESHOLD0 && all(cc.gggg < min(v1, v2))?0.5: "
           "dog;\n";
    ...
    out << "	if(dog == 0.0) return;\n"
           ...
           "	if( score_down <= 0 || score_up > THRESHOLD2 * "
           "score_down)return;\n"
        //...
        << "	float2 D5 = 0.5*(v1.yw-v1.xz); \n"
    ...
    if (GlobalUtil::_SubpixelLocalization)
        ...
        out << "	fs = 0.5*( v3.x - v3.y ); //bug fix 9/12/2007 \n"
               "	fss = v3.x + v3.y - cc.g - cc.g;\n"
               "	fxs = 0.25 * ( v4.y + v5.x - v4.x - v5.y);\n"
               "	fys = 0.25 * ( v4.w + v5.z - v4.z - v5.w);\n"
            ...
            << "	bool bugfix_test = (abs(cc.g + 0.5*dot(float3(fx, fy, "
               "fs), dxys )) < THRESHOLD1) ;\n"
        ...
        out << "	FragData1 =  float4( dog, 0, 0, 0) ;	\n"
               "}\n"
            << '\0';
    ...
    if (!program->IsValidProgram()) {
        delete program;
        out.seekp(pos);
        out << "	FragData1 =  float4( fabs(cc.g) > 2.0 * THRESHOLD0? "
               "dog : 0, 0, 0, 0) ;	\n"
               "}\n"
            << '\0';
        s_keypoint = program = new ProgramCG(buffer);
...
ProgramGPU *FilterGLCG::CreateFilterH(float kernel[], float offset[],
                                      int width) {
    ...
    if (GlobalUtil::_BetaFilter) {
        ...
        for (int i = 0; i < width; i += 4) {
            ...
            for (int j = i; j < i + 4; j++) {
                ...
                if (j >= width) {
                    ...
                    out << "texRECT(tex, TexCoord0.xy + float2(float("
                        << offset[j] << ") , 0)).r";
                }
            }
            out << ");\n";
        ...
        for (int i = 0; i < width; i++) {
            if (offset[i] == 0.0) {
                ...
                out << "pos = TexCoord0.xy + float2(float(" << offset[i]
                    << ") , 0);\n";
...
ProgramGPU *FilterGLCG::CreateFilterV(float kernel[], float offset[],
                                      int height) {
    char buffer[10240];
    ostrstream out(buffer, 10240);
    out << setprecision(8);
        ...
        for (int i = 0; i < height; i += 4) {
            ...
            for (int j = i; j < i + 4; j++) {
                ...
                if (j >= height) {
                    ...
                    out << "texRECT(tex, TexCoord0.xy + float2(0, float("
                        << offset[j] << "))).b";
                }
            }
            out << ");\n";
...
ProgramGPU *FilterGLCG::CreateFilterHPK(float kernel[], float offset[],
                                        int width) {
    ...
    for (i = 0; i < npixel; i++) {
        ...
        // out<<"FragColor +=
        // float4("<<weight[2]<<","<<weight[0]<<","<<weight[2]<<","<<weight[0]<<")*pc.grab;\n";

        if (weight[1] == 0.0) {
            out << "result += float4(" << weight[2] << "," << weight[0] << ","
                << weight[2] << "," << weight[0] << ")*pc.grab;\n";
        } else {
            out << "result += float4(" << weight[1] << ", " << weight[0] << ", "
                << weight[1] << ", " << weight[0] << ")*pc.rrbb;\n";
            out << "result += float4(" << weight[2] << ", " << weight[1] << ", "
...
ProgramGPU *FilterGLCG::CreateFilterVPK(float kernel[], float offset[],
                                        int height) {
    ...
    for (i = 0; i < npixel; i++) {
        ...
        // if(weight[1]!=0.0)	out<<"FragColor += "<<weight[1]<<"*pc;\n";
        // out<<"FragColor +=
        // float4("<<weight[2]<<","<<weight[2]<<","<<weight[0]<<","<<weight[0]<<")*pc.barg;\n";
        if (weight[1] == 0.0) {
            out << "result += float4(" << weight[2] << "," << weight[2] << ","
                << weight[0] << "," << weight[0] << ")*pc.barg;\n";
        } else {
            out << "result += float4(" << weight[1] << "," << weight[1] << ","
...
void ShaderBagCG::LoadGenListShader(int ndoglev, int nlev) {
    ...
    s_genlist_init_tight = new ProgramCG(
        ...
        "in float4 TexCoord1 : TEXCOORD1, in float4 TexCoord2 : TEXCOORD2, in "
        "float4 TexCoord3 : TEXCOORD3,\n"
        "out float4 FragColor : COLOR0){\n"
        "float4 helper = float4( texRECT(tex, TexCoord0.xy).r,  texRECT(tex, "
        "TexCoord1.xy).r,\n"
    ...
    s_genlist_histo = new ProgramCG(
        ...
        "in float2 TexCoord1 : TEXCOORD1, in float2 TexCoord2 : TEXCOORD2, in "
    ...
    if (step > 0) {
        ...
        for (i = 0; i < step; i++) {
            ...
            /*			out<<"cc = texRECT(tex"<<i<<", pos);\n";
                                    out<<"float sum3[3] = {cc.r, cc.r + cc.g,
               cc.r + cc.g + cc.b};\n"; out<<"float3 cmp = float3(index >
               float3(sum3[0], sum3[1], sum3[2]));\n"; out<<"opos.y = -0.5 +
               cmp.y; opos.x = -0.5 + cmp.x + (cmp.z - cmp.y);\n"; out<<"index
               -= dot(cmp, cc.rgb);\n"; out<<"pos = (pos + pos + opos);\n";*/

            out << "cc = texRECT(tex" << i << ", pos); sum.x = cc.r + cc.g;\n";
            out << "if (index < sum.x){ if(index < cc.r) opos = cpos.xx; else "
                   "{opos = cpos.yx; index -= cc.r;}}\n";
            out << "else {index -= sum.x; if(index < cc.b) opos = cpos.xy; "
                   "else{opos = cpos.yy; index -= cc.b;}}";
            out << "pos = (pos + pos + opos);\n";
            ...
            /*			out<<"cc = texRECT(tex"<<i<<", pos);\n";
                                    ...
                                    out<<"else{sum.x = cc.r + cc.g;";
                                                    out<<"if(index < sum.x )
               {opos = cpos.yx; index -= cc.r;}\n"; out<<"else{sum.y = sum.x +
               cc.b;"; out<<"if(index < sum.y ) {opos = cpos.xy; index -=
            ...
            out << "cc = texRECT(tex" << i << ", pos);\n";
            out << "if (index < cc.r) opos = cpos.xx;\n";
            out << "else if (index < cc.r + cc.g){opos = cpos.yx; index -= "
                   "cc.r;}\n";
            out << "else if (index < cc.r + cc.g + cc.b){opos = cpos.xy; index "
                   "-= (cc.r + cc.g);}\n";
...
void ShaderBagCG::LoadDescriptorShaderF2() {
    ...
    out << "\n"
           ...
           "	\n"
           "	float index = dsize.x * floor(TexCoord0.y * 0.5) + "
           "TexCoord0.x;\n"
           "	float idx = 8.0 * frac(index * 0.125) + 8.0 * floor(2.0 * "
           "frac(TexCoord0.y * 0.5));		\n"
           "	index = floor(index*0.125) + 0.49;  \n"
           "	float2 coord = floor( float2( fmod(index, dsize.z), "
           "index*dsize.w)) + 0.5 ;\n"
    ...
    out << "	float4 temp; float2 pt, offsetpt;			"
           "	\n"
    ...
    out << "\n"
           ...
           "			{\n"
           "				float4 cc = texRECT(gradTex, "
           "spos);						\n"
           "				float mod = cc.b;	float angle = "
           "cc.a;					\n"
           "				float theta0 = (anglef - "
           "angle)*RPI;				\n"
           "				float theta = theta0 < 0? theta0 + 8.0 "
           ": theta0; // fmod(theta0 + 8.0, 8.0); \n"
           ...
           "				DA += float4(theta1 == float4(0, 1, 2, "
           "3))*weight1; \n"
           "				DA += float4(theta1 == float4(7, 0, 1, "
           "2))*weight2; \n"
           "				DB += float4(theta1 == float4(4, 5, 6, "
           "7))*weight1;	\n"
           "				DB += float4(theta1 == float4(3, 4, 5, "
           "6))*weight2; \n"
           "			}\n"
           "		}\n"
           "	}\n";
...
void ShaderBagCG::LoadOrientationShader() {
    ...
    // when we use one 32bit float to store two orientations, no extra texture
    // is required
    ...
    out << "	//bool fixed_orientation = (size.z < 0);		\n"
           ...
           "		\n"
           "	const float2 win = abs(sigma.xx) * (SAMPLE_WF * "
           "GAUSSIAN_WF);	\n"
    ...
    out << "\n"
           ...
           "					\n"
           "				const float4 cc = texRECT(gradTex, "
           "spos);			\n"
           "				const float grad = cc.b;	float "
           "theta = cc.a;		\n"
           "				float idx = "
           "floor(degrees(theta)*0.1);		\n"
           "				const float weight = grad*exp(sq_dist "
           "* factor);				\n"
           "				if(idx < 0 ) idx += 36;		"
           "							\n"
           "				const float vidx = 4.0 * fract(idx * "
           "0.25);//fmod(idx, 4);					"
           "			\n"
           "				const float4 inc = weight*float4(vidx "
           "== float4(0,1,2,3));	";

    if (GlobalUtil::_UseDynamicIndexing &&
        strcmp(cgGetProfileString(ProgramCG::_FProfile), "gp4fp") == 0)
    //	if(ProgramCG::_FProfile == CG_PROFILE_GPU_FP) this enumerant is not
    // defined in cg1.5
    {
...
void ShaderBagCG::WriteOrientationCodeToStream(std::ostream &out) {
    ...
    */
    ...
    // bug fixed here:)
    out << "\n"
           "	float3x3 mat1 = float3x3(1, 0, 0, 3, 1, 0, 6, 3, 1)/27.0;; "
           "//bug fix.. \n"
           "	float4x4 mat2 = float4x4( 7, 6, 3, 1, 6, 7, 6, 3, 3, 6, 7, 6, "
           "1, 3, 6, 7)/27.0;;\n"
           "	for (int j=0; j<2; j++)					"
           "			\n"
           "	{							"
           ...
           "		{						"
           "						\n"
           "			float4 newb	=	mul ( bins[i], "
           "mat2);		\n"
           "			newb.xyz	+=	mul ( prev.yzw, "
           "mat1);		\n"
           "			prev = bins[i];				"
           "				\n"
           "			newb.wzy	+=	mul	( "
    ...
    out << "\n"
           ...
           "			\n"
           "	maxh2 = max(maxh4.xy, maxh4.zw); maxh = float4(max(maxh2.x, "
           "maxh2.y));";
    ...
    if (GlobalUtil::_MaxOrientation > 1) {
        ...
        if (GlobalUtil::_MaxOrientation == 2) {
            savepeak_code =
                ...
                "		{weights.rg = float2(weight, weights.r); "
                "Orientations.rg = float2(th, Orientations.r);}\n"
                "		else {weights.g = weight; Orientations.g = "
                "th;}";
            ...
            savepeak_code =
                ...
                "		if(weight <=weights.b){}\n"
                "		else if(weight >weights.r)\n"
                "		{weights.rgb = float3(weight, weights.rg); "
                "Orientations.rgb = float3(th, Orientations.rg);}\n"
                "		else if(weight >weights.g)\n"
                "		{weights.gb = float2(weight, weights.g); "
                "Orientations.gb = float2(th, Orientations.g);}\n"
                "		else {weights.b = weight; Orientations.b = "
                "th;}";
        } else {
            savepeak_code =
                "\n"
                "		if(weight <=weights.a){}\n"
                "		else if(weight >weights.r)\n"
                "		{weights = float4(weight, weights.rgb); "
                "Orientations = float4(th, Orientations.rgb);}\n"
                "		else if(weight >weights.g)\n"
                "		{weights.gba = float3(weight, weights.gb); "
                "Orientations.gba = float3(th, Orientations.gb);}\n"
                "		else if(weight >weights.b)\n"
                "		{weights.ba = float2(weight, weights.b); "
                "Orientations.ba = float2(th, Orientations.b);}\n"
                "		else {weights.a = weight; Orientations.a = "
                "th;}";
        }
    ...
    out << "\n"
           "	const float4 hh = maxh * ORIENTATION_THRESHOLD;	bool4 "
           "test;	\n"
           "	bins[9] = bins[0];					"
           "			\n"
           "	float npeaks = 0, k = 0;			\n"
        ...
        << "								"
           ...
           "(bins[i].x *2.0 -bins[i].y -prevb) ; \n"
           "				float	th = (k+di+0.5);	float "
           "weight = bins[i].x;"
        << savepeak_code
        << "\n"
           "			}\n"
           "			else if(test.g && all( bins[i].yy > "
           "bins[i].xz) )	\n"
           "			{					"
           "						\n"
           "			    float	di = 0.5 * "
           "(bins[i].z-bins[i].x) / (bins[i].y * 2.0 - bins[i].z - bins[i].x) "
           "; \n"
           "				float	th = (k+di+1.5);	float "
           "weight = bins[i].y;				"
        << savepeak_code
        << "	\n"
           "			}"
        << "\n"
           ...
           "						\n"
           "			    float	di = 0.5 * "
           "(bins[i].w-bins[i].y) / (bins[i].z * 2.0-bins[i].w-bins[i].y) ; \n"
           "				float	th = (k+di+2.5);	float "
           "weight = bins[i].z;				"
        << savepeak_code
        << "	\n"
           "			}\n"
           "			else if(test.a && bins[i].w > bins[i].z && "
           "bins[i].w > bins[i+1].x )	\n"
           "			{					"
           "						\n"
           "			    float	di = 0.5 * "
           "(bins[i+1].x-bins[i].z) / (bins[i].w * 2.0- bins[i+1].x-bins[i].z) "
           "; \n"
           "				float	th = (k+di+3.5);	float "
           "weight = bins[i].w;				"
        << savepeak_code
        << "	\n"
           "			}\n"
    ...
    if (GlobalUtil::_OrientationPack2) {
        ...
        out << "\n"
               "	 if(orientation_mode){\n"
               "		Orientations.xy = frac(Orientations.xy / 36.0 "
               "+ 1.0);\n"
               "		if(weights.x <= 0) Orientations.x = 1.0;\n"
               "		if(weights.y <= 0) Orientations.y = 1.0;\n"
               "		float packed_orientation = "
               "pack_2ushort(Orientations.xy); \n"
               "		FeatureData = float4(pos, packed_orientation, "
               "sigma);\n"
               "	}else{\n"
               "		FeatureData = float4(pos, "
        ...
        out << "\n"
            ...
            << "), float4(weights>hh));\n"
               "		OrientationData = radians((Orientations "
               ")*10.0);\n"
...
void ShaderBagCG::SetSimpleOrientationInput(int oTex, float sigma,
                                            float sigma_step) {
...
void ShaderBagCG::SetFeatureOrientationParam(int gtex, int width, int height,
...
void ShaderBagCG::SetFeatureDescirptorParam(int gtex, int otex, float dwidth,
...
ShaderBagPKCG::ShaderBagPKCG() { ProgramCG::InitContext(); }
...
void ShaderBagPKCG::LoadFixedShaders() {
    ...
    char *rgb2gray_packing_code =
            "void main(uniform samplerRECT rgbTex, in float4 TexCoord0 :
    TEXCOORD0, \n" "		in float4 TexCoord1 : TEXCOORD1, in float4
    TexCoord2 : TEXCOORD2, \n"
    "		in float4 TexCoord3 : TEXCOORD3, out float4 FragData :
    COLOR0){\n" "		const float3 weight = vec3(0.299, 0.587,
    0.114);\n"
    ...
    "		FragData.a = dot(weight, texRECT(rgbTex,TexCoord3.st
    ).rgb);}";// s_gray = new ProgramCG( rgb2gray_packing_code);
    */
    ...
    s_sampling = new ProgramCG(
        ...
        "		in float4 TexCoord1 : TEXCOORD1, in float4 TexCoord2 : "
        "TEXCOORD2, \n"
        "		in float4 TexCoord3 : TEXCOORD3, out float4 FragData : "
        "COLOR0 ){\n"
        "		FragData= float4(	texRECT(tex,TexCoord0.st "
        ").r,texRECT(tex,TexCoord1.st ).r,\n"
        "							"
        "texRECT(tex,TexCoord2.st ).r,texRECT(tex,TexCoord3.st ).r);}");

    s_margin_copy = program = new ProgramCG(
        "void main(in float4 texCoord0: TEXCOORD0, out float4 FragColor: "
        "COLOR0, \n"
    ...
    s_grad_pass = program = new ProgramCG(
        ...
        "TEXCOORD4,\n"
        "out float4 FragData0 : COLOR0, out float4 FragData1 : COLOR1, \n"
        "out float4 FragData2 : COLOR2, uniform samplerRECT tex, uniform "
        "samplerRECT texp)\n"
        "{\n"
        "	float4 v1, v2, gg;\n"
        "	float4 cc = texRECT(tex, TexCC.xy);\n"
        ...
        "	FragData0 = cc - cp; \n"
        "	float4 cl = texRECT(tex, TexLC.xy);	float4 cr = "
        "texRECT(tex, TexRC.xy);\n"
        "	float4 cd = texRECT(tex, TexCD.xy); float4 cu = texRECT(tex, "
        "TexCU.xy);\n"
        "	float4 dx = (float4(cr.rb, cc.ga) - float4(cc.rb, "
        "cl.ga)).zxwy;\n"
        "	float4 dy = (float4(cu.rg, cc.ba) - float4(cc.rg, "
        "cd.ba)).zwxy;\n"
...
void ShaderBagPKCG::LoadDisplayShaders() {
    ...
    s_copy_key =
        new ProgramCG("void main(in float4 TexCoord0 : TEXCOORD0, out float4 "
                      "FragColor : COLOR0, uniform samplerRECT tex){\n"
                      "FragColor.rg= texRECT(tex, TexCoord0.xy).rg; "
                      "FragColor.ba = float2(0,1);	}");
    ...
    s_display_gaussian =
        new ProgramCG("void main(uniform samplerRECT tex, in float4 "
                      "TexCoord0:TEXCOORD0, out float4 FragData: COLOR0 ){\n"
                      "float4 pc = texRECT(tex, TexCoord0.xy);	 bool2 ff = "
                      "(frac(TexCoord0.xy) < 0.5);\n"
                      "float v = ff.y?(ff.x? pc.r : pc.g):(ff.x?pc.b:pc.a); "
                      "FragData = float4(v.xxx, 1.0);}");
    ...
    s_display_dog =
        new ProgramCG("void main(in float4 TexCoord0 : TEXCOORD0, out float4 "
                      ...
                      "float4 pc = texRECT(tex, TexCoord0.xy); bool2 ff = "
                      "(frac(TexCoord0.xy) < 0.5);\n"
                      "float v = ff.y ?(ff.x ? pc.r : pc.g):(ff.x ? pc.b : "
                      "pc.a);float g = (0.5+20.0*v);\n"
                      "FragColor = float4(g, g, g, 1.0);}");
    ...
    s_display_grad =
        new ProgramCG("void main(in float4 TexCoord0 : TEXCOORD0, out float4 "
                      ...
                      "(frac(TexCoord0.xy) < 0.5);\n"
                      "float v = ff.y ?(ff.x ? pc.r : pc.g):(ff.x ? pc.b : "
                      "pc.a); FragColor = float4(5.0 *v.xxx, 1.0); }");
...
void ShaderBagPKCG::LoadGenListShader(int ndoglev, int nlev) {
    ...
    s_genlist_init_ex = program = new ProgramCG(
        ...
        "out float4 FragColor : COLOR0)\n"
        "{\n"
        "	bool4 helper1 = abs(texRECT(tex, TexCoord0.xy).r)== "
        "float4(1.0, 2.0, 3.0, 4.0); \n"
        "	bool4 helper2 = abs(texRECT(tex, TexCoord1.xy).r)== "
        "float4(1.0, 2.0, 3.0, 4.0);\n"
        "	bool4 helper3 = abs(texRECT(tex, TexCoord2.xy).r)== "
        "float4(1.0, 2.0, 3.0, 4.0);\n"
        "	bool4 helper4 = abs(texRECT(tex, TexCoord3.xy).r)== "
        "float4(1.0, 2.0, 3.0, 4.0);\n"
    ...
    s_genlist_end = program = new ProgramCG(
        GlobalUtil::_KeepExtremumSign == 0
            ...
            "			in float4 tpos : TEXCOORD0,	out float4 "
            "FragColor : COLOR0)\n"
            "{\n"
            ...
            "	opos.x = dot(keys, float4(-0.5, 0.5, -0.5, 0.5));\n"
            "	opos.y = dot(keys, float4(-0.5, -0.5, 0.5, 0.5));\n"
            "	FragColor = float4(opos + pos + pos + tk.yz, sign(tk.x), "
            "tk.w);\n"
            "}");
    ...
    s_genlist_histo = new ProgramCG(
        ...
        "{\n"
        "	float4 helper; float4 helper2; \n"
        "	helper = texRECT(tex, TexCoord0); helper2.xy = helper.xy + "
        "helper.zw; \n"
        "	helper = texRECT(tex, TexCoord1); helper2.zw = helper.xy + "
        "helper.zw; \n"
        "	FragColor.rg = helper2.xz + helper2.yw;\n"
        "	helper = texRECT(tex, TexCoord2); helper2.xy = helper.xy + "
        "helper.zw; \n"
        "	helper = texRECT(tex, TexCoord3); helper2.zw = helper.xy + "
        "helper.zw; \n"
        "	FragColor.ba= helper2.xz+helper2.yw;\n"
        "}");
...
void ShaderBagPKCG::LoadGenListShaderV2(int ndoglev, int nlev) {
    ...
    s_genlist_init_tight = new ProgramCG(
        ...
        "		if(FragColor.r){ \n"
        "			data = data1; helper = helper1; pos = "
        "TexCoord0.xy;\n"
        "		}else if(FragColor.g){\n"
        "			data = data2; helper = helper2; pos = "
        "TexCoord1.xy;\n"
        "		}else if(FragColor.b){\n"
        "			data = data3; helper = helper3; pos = "
        "TexCoord2.xy;\n"
        "		}else{\n"
        "			data = data4; helper = helper4; pos = "
        "TexCoord3.xy;\n"
        "		}\n"
        "		opos.x = dot(helper, float4(-0.5, 0.5, -0.5, 0.5));\n"
        "		opos.y = dot(helper, float4(-0.5, -0.5, 0.5, 0.5));\n"
        "		FragColor = float4( pos + pos + opos + data.yz, -1, "
        "data.w); \n"
        "	}\n"
        "}");
    ...
    s_genlist_init_ex = program = new ProgramCG(
        ...
        "\n"
        "		if(FragColor.r){ \n"
        "			data = data1; bhelper = helper1; pos = "
        "TexCoord0.xy;\n"
        "		}else if(FragColor.g){\n"
        "			data = data2; bhelper = helper2; pos = "
        "TexCoord1.xy;\n"
        "		}else if(FragColor.b){\n"
        "			data = data3; bhelper = helper3; pos = "
        "TexCoord2.xy;\n"
        "		}else{\n"
        "			data = data4; bhelper = helper4; pos = "
        "TexCoord3.xy;\n"
        "		}\n"
    ...
    s_genlist_end = program = new ProgramCG(
        ...
        "	{\n"
        "		float4 tk = texRECT( ktex, pos); \n"
        "		float4 keys = float4(abs(tk.r) == float4(1.0, 2.0, "
        "3.0, 4.0)); \n"
        "		float2 opos; \n"
        "		opos.x = dot(keys, float4(-0.5, 0.5, -0.5, 0.5));\n"
        "		opos.y = dot(keys, float4(-0.5, -0.5, 0.5, 0.5));\n"
        "		FragColor = float4(opos + pos + pos + tk.yz, 0, "
        "tk.w);\n"
        "	}\n"
        "}");
    _param_genlist_end_ktex = cgGetNamedParameter(*program, "ktex");
    ...
    s_genlist_histo = new ProgramCG(
        ...
        "	helper[0] = texRECT(tex, TexCoord0); helper2.xy = helper[0].xy "
        "+ helper[0].zw; \n"
        "	helper[1] = texRECT(tex, TexCoord1); helper2.zw = helper[1].xy "
        "+ helper[1].zw; \n"
        "	FragColor.rg = helper2.xz + helper2.yw;\n"
        "	helper[2] = texRECT(tex, TexCoord2); helper2.xy = helper[2].xy "
        "+ helper[2].zw; \n"
        "	helper[3] = texRECT(tex, TexCoord3); helper2.zw = helper[3].xy "
        "+ helper[3].zw; \n"
        "	FragColor.ba= helper2.xz+helper2.yw;\n"
        "	bool4 keyt = float4(helper[0].z, helper[1].z, helper[2].z, "
        "helper[3].z) == -1.0; \n"
...
ProgramCG *ShaderBagCG::LoadGenListStepShaderV2(int start, int step) {
    ...
    if (step > 0) {
        ...
        for (i = 0; i < step; i++) {
            ...
            out << "sum.x = cc.r + cc.g;if (index < sum.x){ if(index < cc.r) "
                   "opos = cpos.xx; else {opos = cpos.yx; index -= cc.r;}}\n";
            out << "else {index -= sum.x; if(index < cc.b) opos = cpos.xy; "
                   "else{opos = cpos.yy; index -= cc.b;}}";
            out << "pos = (pos + pos + opos);\n";
        }
...
void ShaderBagPKCG::LoadKeypointShader(float threshold, float edge_threshold) {
    ...
    out << "void main (\n"
        ...
        << "	float4 key ={0, 0, 0, 0}; \n"
           "	for(int i = 0; i < 4; i++)\n"
           "	{\n"
           "		bool4 test1 = cc[i] > max(v1[i], v2[i]), test2 = cc[i] "
           "< min(v1[i], v2[i]);\n"
           "		key[i] = cc[i] > THRESHOLD0 && "
           "all(test1.xy&&test1.zw)?1.0: 0.0;\n"
           "		key[i] = cc[i] < -THRESHOLD0 && "
           "all(test2.xy&&test2.zw)? -1.0: key[i];\n"
           "	}\n"
    ...
    if (GlobalUtil::_SubpixelLocalization)
        ...
        out << "	float3 offset = float3(0, 0, 0); \n"
               "	/*The unrolled following loop is faster than a dynamic "
               "indexing version.*/\n"
            ...
            "offset.y*A0.y;	\n"
            "				bool test = (abs(cc[0] + "
            "0.5*dot(float3(fx[0], fy[0], fs), offset ))>THRESHOLD1) ;\n"
            "				if(!test || any( abs(offset) >= 1.0)) "
            "return;\n"
            "			}\n"
            "		}\n"
            "	}\n"
...
void ShaderBagPKCG::LoadOrientationShader() {
    ...
    out << "\n"
        ...
        << GlobalUtil::_MulitiOrientationThreshold
        << "\n"
           "void main(uniform samplerRECT tex,	uniform samplerRECT "
           "gtex,		\n"
           "		uniform samplerRECT otex, 	uniform float4 size, "
           "in float2 TexCoord0 : TEXCOORD0,	\n"
           "		out float4 FeatureData : COLOR0	";
    ...
    out << "\n"
           ...
           "				\n"
           "			float4 off = float4(offset, offset + "
           "1);				\n"
           "			float4 distsq = off.xzxz * off.xzxz + off.yyww "
           "* off.yyww;	\n"
           ...
           "								"
           "	\n"
           "				float4 gg = texRECT(gtex, "
           "spos);					\n"
           "				float4 oo = texRECT(otex, "
           "spos);					\n"
           "				float4 weight = gg * exp(distsq * "
           "factor);			\n"
           "				float4 idxv  = floor(degrees(oo)*0.1); "
           "				\n"
           "				idxv = idxv<0? idxv + 36.0: "
           "idxv;					\n"
           "				float4 vidx = 4.0* fract(idxv * "
           "0.25);//fmod(idxv, 4.0);\n";
    ...
    // defined in cg1.5
    {
        ...
        out << "\n"
               ...
               "		\n"
               "						float4 inc = "
               "weight[i] * float4(vidx[i] == float4(0,1,2,3));	\n"
               "						int iidx = "
               "int(floor(idx*0.25));	\n"
               "						"
               "bins[iidx]+=inc;					\n"
...
void ShaderBagPKCG::LoadDescriptorShaderF2() {
    ...
    out << "	float2 bwin = abs(cscs.xy);				"
           ...
           "	sz = floor(sz * 0.5) + 0.5;"; // move sample point to pixel
                                              // center
    // get voting for two box
    ...
    out << "\n"
           ...
           "				float4 theta = theta0 < 0? theta0 + "
           "8.0 : theta0;//8.0 * frac(1.0 + 0.125 * theta0);// 		\n"
           "				float4 theta1 = floor(theta); \n"
           "				float4 diffx = nx + offsetpt.x, diffy "
           "= ny + offsetpt.y;	\n"
           "				float4 ww = exp(-0.125 * (diffx * "
           "diffx + diffy * diffy ));	\n"
           "				float4 weight = (1 - nxn) * (1 - nyn) "
           "* gg * ww; \n"
           ...
           "					{\n"
           "						DA += float4(theta1[i] "
           "== float4(0, 1, 2, 3))*weight1[i];	\n"
           "						DA += float4(theta1[i] "
           "== float4(7, 0, 1, 2))*weight2[i];	\n"
           "						DB += float4(theta1[i] "
           "== float4(4, 5, 6, 7))*weight1[i];	\n"
           "						DB += float4(theta1[i] "
           "== float4(3, 4, 5, 6))*weight2[i];	\n"
           "					}\n"
           "				}\n"
           "			}\n"
           "		}\n"
...
void ShaderBagPKCG::SetSimpleOrientationInput(int oTex, float sigma,
                                              float sigma_step) {
    cgGLSetTextureParameter(_param_orientation_gtex, oTex);
    cgGLEnableTextureParameter(_param_orientation_gtex);
...
void ShaderBagPKCG::SetFeatureOrientationParam(int gtex, int width, int height,
...
void ShaderBagPKCG::SetFeatureDescirptorParam(int gtex, int otex, float dwidth,
                                              float fwidth, float width,
                                              float height, float sigma) {
    ///
