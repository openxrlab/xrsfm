...
void SiftPyramid::LimitFeatureCount(int have_keylist) {
    ...
    if (GlobalUtil::_TruncateMethod == 2) {
        int i = 0, new_feature_num = 0,
            level_num = param._dog_level_num * _octave_num;
        ...
        if (num_to_erase > 0 && have_keylist) {
            _keypoint_buffer.erase(_keypoint_buffer.begin(),
                                   _keypoint_buffer.begin() + num_to_erase * 4);
        }
...
void SiftPyramid::PrepareBuffer() {
    // when there is no existing keypoint list, the feature list need to be
    // downloaded when an existing keypoint list does not have orientaiton, we
    // need to download them again.
    if (!(_existing_keypoints & SIFT_SKIP_ORIENTATION)) {
        //_keypoint_buffer.resize(4 * (_featureNum +align));
        _keypoint_buffer.resize(
            4 * (_featureNum + GlobalUtil::_texMaxDim)); // 11/19/2008
    }
    if (GlobalUtil::_DescriptorPPT) {
        //_descriptor_buffer.resize(128*(_featureNum + align));
        _descriptor_buffer.resize(128 * _featureNum +
                                  16 * GlobalUtil::_texMaxDim); // 11/19/2008
    }
}
...
int SiftPyramid::GetRequiredOctaveNum(int inputsz) {
    ...
    int num =
        (int)floor(log(inputsz * 2.0 / GlobalUtil::_texMinDim) / log(2.0));
    return num <= 0 ? 1 : num;
}
...
void SiftPyramid::CopyFeatureVector(float *keys, float *descriptors) {
    ...
    if (descriptors)
        memcpy(descriptors, &_descriptor_buffer[0],
               128 * _featureNum * sizeof(float));
}

void SiftPyramid::SetKeypointList(int num, const float *keys,
                                  int run_on_current, int skip_orientation) {
    ...
    if (GlobalUtil::_BinarySIFT) {
        ...
        if (GlobalUtil::_DescriptorPPT) {
            ...
            for (int i = 0; i < _featureNum; i++) {
                ...
                out << setprecision(2) << pk[1] << " " << setprecision(2)
                    << pk[0] << " " << setprecision(3) << pk[2] << " "
                    << setprecision(3) << pk[3] << endl;

                ////out << setprecision(12) << pk[1] <<  " " << pk[0] << " " <<
                /// pk[2] << " " << pk[3] << endl;
                pk += 4;
...
void SiftPyramid::StopDEBUG() { _debug_path[0] = 0; }
...
void SiftPyramid::WriteTextureForDEBUG(GLTexImage *tex, const char *namet,
    ...
    *p++ = '/';
    ...
    for (int i = 0; i < height; i++) {
        memcpy(buffer2 + i * width * 4, buffer1 + (height - i - 1) * width * 4,
