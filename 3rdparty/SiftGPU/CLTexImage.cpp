...
CLTexImage::~CLTexImage() { ReleaseTexture(); }
...
void CLTexImage::InitBufferTex(int width, int height, int nchannel) {
    ...
    _clData = clCreateBuffer(_context, CL_MEM_READ_WRITE, _bufferLen = size,
                             NULL, &status);
...
void CLTexImage::InitTexture(int width, int height, int nchannel) {
    if (width == 0 || height == 0 || nchannel <= 0 || _fromGL)
        return;
    if (_clData && width == _texWidth && height == _texHeight &&
    ...
    _clData = clCreateImage2D(_context, CL_MEM_READ_WRITE, &format, _texWidth,
                              _texHeight, 0, 0, &status);
...
void CLTexImage::InitTextureGL(GLuint tex, int width, int height,
                               int nchannel) {
    if (tex == 0)
        return;
    if (_clData)
    ...
    ProgramBagCL::CheckErrorCL(
        status, "CLTexImage::InitTextureGL->clCreateFromGLTexture2D");
    ...
    ProgramBagCL::CheckErrorCL(
        status, "CLTexImage::InitTextureGL->clEnqueueAcquireGLObjects");
}
...
void CLTexImage::CopyFromHost(const void *buf) {
    ...
    if (_bufferLen) {
        status = clEnqueueWriteBuffer(_queue, _clData, false, 0,
                                      _imgWidth * _imgHeight * _numChannel *
                                          sizeof(float),
                                      buf, 0, NULL, NULL);
    } else {
        size_t origin[3] = {0, 0, 0}, region[3] = {_imgWidth, _imgHeight, 1};
        size_t row_pitch = _imgWidth * _numChannel * sizeof(float);
        status = clEnqueueWriteImage(_queue, _clData, false, origin, region,
                                     row_pitch, 0, buf, 0, 0, 0);
    }
...
int CLTexImage::CopyToPBO(GLuint pbo) {
    ...
    if (bsize < esize) {
        glBufferData(GL_PIXEL_UNPACK_BUFFER_ARB, esize, NULL,
                     GL_STATIC_DRAW_ARB);
        glGetBufferParameteriv(GL_PIXEL_UNPACK_BUFFER_ARB, GL_BUFFER_SIZE,
                               &bsize);
    }
    if (bsize >= esize) {
...
void CLTexImage::CopyToHost(void *buf) {
    ...
    if (_bufferLen) {
        ...
        size_t origin[3] = {0, 0, 0}, region[3] = {_imgWidth, _imgHeight, 1};
        size_t row_pitch = _imgWidth * _numChannel * sizeof(float);
        status = clEnqueueReadImage(_queue, _clData, true, origin, region,
                                    row_pitch, 0, buf, 0, 0, 0);
    }
