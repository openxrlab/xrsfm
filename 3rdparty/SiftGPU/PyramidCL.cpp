...
PyramidCL::PyramidCL(SiftParam &sp) : SiftPyramid(sp) {
    ...
    _inputTex =
        new CLTexImage(_OpenCL->GetContextCL(), _OpenCL->GetCommandQueue());
    /////////////////////////
    InitializeContext();
}
...
void PyramidCL::InitializeContext() { GlobalUtil::InitGLParam(1); }
...
void PyramidCL::InitPyramid(int w, int h, int ds) {
    ...
    if (toobig && GlobalUtil::_verbose && _octave_min > 0) {
        std::cout << "*********************************************************"
                     "*****\n"
                     "Image larger than allowed dimension, data will be "
                     "downsampled!\n"
    ...
    _OpenCL->SelectInitialSmoothingFilter(_octave_min + _down_sample_factor,
                                          param);
}
...
int PyramidCL::ResizeFeatureStorage() {
    ...
    for (i = 0; i < _octave_num; i++) {
        CLTexImage *tex = GetBaseLevel(i + _octave_min);
        int fmax = int(4 * tex->GetTexWidth() * tex->GetTexHeight() *
...
void PyramidCL::GetFeatureDescriptors() {
    ...
    for(int i = 0, idx = 0; i < _octave_num; i++)
    {
            ...
            for(int j = 0; j < param._dog_level_num; j++, ftex++, idx++, got++)
            {
                    ...
                    ProgramCL::ComputeDescriptor(ftex, got,
    _descriptorTex);//process _descriptorTex->CopyToHost(pd); //readback
    descriptor pd += 128*_levelFeatureNum[idx];
            }
    }
        ...
        for (int j = 0; j < param._dog_level_num; j++, idx++) {
            ...
            for (int k = 0; k < _featureNum; k++) {
                ...
                if ((key[2] >= sigma_min && key[2] < sigma_max) ||
                    ...
                    (key[2] > sigma_max && i == _octave_num - 1 &&
                     j == param._dog_level_num - 1)) {
...
void PyramidCL::GenerateFeatureDisplayVBO() {
    ...
    for(int i = 0; i < nvbo; i++)
    {
            ...
            CLTexImage texPBO1( _levelFeatureNum[i]* 10, 1, 4,
    _featureDisplayVBO[i]); CLTexImage texPBO2(_levelFeatureNum[i], 1, 4,
    _featurePointVBO[i]); _OpenCL->DisplayKeyBox(ftex, &texPBO1);
...
void PyramidCL::GenerateFeatureList(int i, int j, int reduction_count,
                                    vector<int> &hbuffer) {
    ...
    //build the feature list
        if(fcount > 0)
        {
                ...
                for(ii = 0; ii < len; ++ii)
                {
                        ...
                        {
                                ibuf[0] = x; ibuf[1] = y; ibuf[2] = jj; ibuf[3]
    = 0;
                        }
                }
...
&& _featureNum > GlobalUtil::_FeatureCountThreshold) continue;
...
GLTexImage *PyramidCL::ConvertTexCL2GL(CLTexImage *tex, int dataName) {
    ...
    case DATA_GRAD:
        ...
        break;
    case DATA_KEYPOINT:
        _OpenCL->UnpackImageKEY(
            tex, tex - param._level_num * _pyramid_octave_num, &texCL);
        break;
    default:
        break;
    }
...
GLTexImage *PyramidCL::GetLevelTexture(int octave, int level, int dataName) {
    CLTexImage *tex =
        GetBaseLevel(octave, dataName) + (level - param._level_min);
...
void PyramidCL::ConvertInputToCL(GLTexInput *input, CLTexImage *output) {
    ...
    } else /*if(input->_rgb_converted && input->CopyToPBO(_bufferPBO, ws, hs,
     GL_LUMINANCE))
     {
                 output->InitTexture(ws, hs, 1);
     ...
     }else if(input->CopyToPBO(_bufferPBO, ws, hs))
         {
                 ...
                 output->InitTexture(ws, hs, 1);
                 ProgramCL::ReduceToSingleChannel(output, &texPBO,
     !input->_rgb_converted); }else*/
    {
...
void PyramidCL::BuildPyramid(GLTexInput *input) {
    ...
    for (i = _octave_min; i < _octave_min + _octave_num; i++) {
        ...
        if (i == _octave_min) {
            ...
            _OpenCL->SampleImageD(tex, GetBaseLevel(i - 1) + param._level_ds -
                                           param._level_min);
...
void PyramidCL::DetectKeypointsEX() {
    ...
    for (i = _octave_min; i < _octave_min + _octave_num; i++) {
        ...
        for (j = param._level_min + 2; j < param._level_max;
            ...
            // output, key
            _OpenCL->ComputeKEY(dog, key, param._dog_threshold,
...
void PyramidCL::GetFeatureOrientations() {
    ...
    /*
            ...
            for(int i = 0; i < _octave_num; i++)
            {
                    CLTexImage* got = GetBaseLevel(i + _octave_min, DATA_GRAD) +
       1; CLTexImage* key = GetBaseLevel(i + _octave_min, DATA_KEYPOINT) + 2;

                    for(int j = 0; j < param._dog_level_num; j++, ftex++,
       count++, got++, key++)
                    {
                            if(*count<=0)continue;

                            //if(ftex->GetImgWidth() < *count)
       ftex->InitTexture(*count, 1, 4);

                            sigma = param.GetLevelSigma(j+param._level_min+1);

                            ProgramCL::ComputeOrientation(ftex, got, key, sigma,
       sigma_step, _existing_keypoints);
                    }
            }
