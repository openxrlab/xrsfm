...
template <int LOG_SCALE>
__global__ void DownsampleKernel(float *d_result, int src_width,
                                 int dst_width) {
...
__global__ void DownsampleKernel(float *d_result, int src_width, int dst_width,
                                 const int log_scale) {
    const int dst_col = IMUL(blockIdx.x, FILTERH_TILE_WIDTH) + threadIdx.x;
    if (dst_col >= dst_width)
        return;
...
void ProgramCU::SampleImageD(CuTexImage *dst, CuTexImage *src, int log_scale) {
    ...
    case 1:
        DownsampleKernel<1>
            <<<grid, block>>>((float *)dst->_cuData, src_width, dst_width);
        break;
    case 2:
        DownsampleKernel<2>
            <<<grid, block>>>((float *)dst->_cuData, src_width, dst_width);
...
void __global__ ComputeDOG_Kernel(float *d_dog, float2 *d_got, int width,
                                  int height) {
...
void ProgramCU::ComputeDOG(CuTexImage *gus, CuTexImage *dog, CuTexImage *got) {
    ...
    if (got->_cuData)
        ...
        ComputeDOG_Kernel<<<grid, block>>>((float *)dog->_cuData, width,
                                           height);
}
...
void __global__ ComputeKEY_Kernel(float4 *d_key, int width, int colmax,
                                  int rowmax, float dog_threshold0,
                                  float dog_threshold, float edge_threshold,
...
void __global__ ComputeOrientation_Kernel(
    float4 *d_list, int list_len, int width, int height, float sigma,
    float sigma_step, float gaussian_factor, float sample_factor,
...
#pragma unroll
        ...
        if (fr1 < 0)
            fr1 += 1.0f;
        unsigned short us1 =
            ocount == 0 ? 65535 : ((unsigned short)floor(fr1 * 65535.0f));
...
void __global__ ComputeDescriptor_Kernel(float4 *d_des, int num, int width,
...
template <bool DYNAMIC_INDEXING>
void __global__ ComputeDescriptorRECT_Kernel(float4 *d_des, int num, int width,
                                             int height, float window_factor) {
    const float rpi = 4.0 / 3.14159265358979323846;
    int idx = IMUL(blockIdx.x, blockDim.x) + threadIdx.x;
    int fidx = idx >> 4;
...
void ProgramCU::ComputeDescriptor(CuTexImage *list, CuTexImage *got,
    ...
    if (rect) {
        if (GlobalUtil::_UseDynamicIndexing)
            ComputeDescriptorRECT_Kernel<true>
                <<<grid, block>>>((float4 *)dtex->_cuData, num, width, height,
                                  GlobalUtil::_DescriptorWindowFactor);
        else
            ComputeDescriptorRECT_Kernel<false>
                <<<grid, block>>>((float4 *)dtex->_cuData, num, width, height,
                                  GlobalUtil::_DescriptorWindowFactor);

    } else {
        if (GlobalUtil::_UseDynamicIndexing)
            ComputeDescriptor_Kernel<true>
                <<<grid, block>>>((float4 *)dtex->_cuData, num, width, height,
                                  GlobalUtil::_DescriptorWindowFactor);
        else
            ComputeDescriptor_Kernel<false>
                <<<grid, block>>>((float4 *)dtex->_cuData, num, width, height,
                                  GlobalUtil::_DescriptorWindowFactor);
    }
