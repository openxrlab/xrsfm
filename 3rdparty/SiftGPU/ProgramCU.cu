...
//////////////////////////////////////////////////////////////
template <int FW>
__global__ void FilterH(cudaTextureObject_t texData, float *d_result,
                        int width) {
...
////////////////////////////////////////////////////////////////////
template <int FW>
__global__ void FilterV(cudaTextureObject_t texData, float *d_result, int width,
                        int height) {
    const int HALF_WIDTH = FW >> 1;
...
#error
    ...
    const int WRITE_COUNT =
        (FILTERV_TILE_HEIGHT + FILTERV_BLOCK_HEIGHT - 1) / FILTERV_BLOCK_HEIGHT;
        ...
        for (int i = 0; i < CACHE_COUNT; ++i) {
            if (cache_col_start < CACHE_WIDTH - i * FILTERV_BLOCK_HEIGHT) {
                ...
                data[cache_index + i * FILTERV_BLOCK_HEIGHT] =
                    tex1Dfetch<float>(texData, fetch_index);
...
__global__ void UpsampleKernel(cudaTextureObject_t texData, float *d_result,
                               int width) {
...
void ProgramCU::SampleImageU(CuTexImage *dst, CuTexImage *src, int log_scale) {
    ...
    CuTexImage::CuTexObj srcTex =
        src->BindTexture(texDataDesc, cudaCreateChannelDesc<float>());
    ...
    case 1:
        UpsampleKernel<1>
            <<<grid, block>>>(srcTex.handle, (float *)dst->_cuData, width);
        break;
    case 2:
        UpsampleKernel<2>
            <<<grid, block>>>(srcTex.handle, (float *)dst->_cuData, width);
...
template <int LOG_SCALE>
__global__ void DownsampleKernel(cudaTextureObject_t texData, float *d_result,
...
void ProgramCU::SampleImageD(CuTexImage *dst, CuTexImage *src, int log_scale) {
    ...
    switch (log_scale) {
    case 1:
        DownsampleKernel<1><<<grid, block>>>(
            srcTex.handle, (float *)dst->_cuData, src_width, dst_width);
        break;
    case 2:
        DownsampleKernel<2><<<grid, block>>>(
            srcTex.handle, (float *)dst->_cuData, src_width, dst_width);
        break;
    case 3:
        DownsampleKernel<3><<<grid, block>>>(
            srcTex.handle, (float *)dst->_cuData, src_width, dst_width);
        break;
    default:
        DownsampleKernel<<<grid, block>>>(srcTex.handle, (float *)dst->_cuData,
                                          src_width, dst_width, log_scale);
    }
}

__global__ void ChannelReduce_Kernel(cudaTextureObject_t texData,
                                     float *d_result) {
    int index = IMUL(blockIdx.x, FILTERH_TILE_WIDTH) + threadIdx.x;
    d_result[index] = tex1Dfetch<float>(texData, index * 4);
}

__global__ void ChannelReduce_Convert_Kernel(cudaTextureObject_t texDataF4,
                                             float *d_result) {
    int index = IMUL(blockIdx.x, FILTERH_TILE_WIDTH) + threadIdx.x;
...
void ProgramCU::ReduceToSingleChannel(CuTexImage *dst, CuTexImage *src,
                                      int convert_rgb) {
    ...
    if (convert_rgb) {
        CuTexImage::CuTexObj srcTex =
            src->BindTexture(texDataDesc, cudaCreateChannelDesc<float4>());
        ChannelReduce_Convert_Kernel<<<grid, block>>>(srcTex.handle,
                                                      (float *)dst->_cuData);
    } else {
        CuTexImage::CuTexObj srcTex =
            src->BindTexture(texDataDesc, cudaCreateChannelDesc<float>());
        ChannelReduce_Kernel<<<grid, block>>>(srcTex.handle,
                                              (float *)dst->_cuData);
    }
}

__global__ void ConvertByteToFloat_Kernel(cudaTextureObject_t texDataB,
                                          float *d_result) {
    int index = IMUL(blockIdx.x, FILTERH_TILE_WIDTH) + threadIdx.x;
...
void ProgramCU::ConvertByteToFloat(CuTexImage *src, CuTexImage *dst) {
    ...
    dim3 block(FILTERH_TILE_WIDTH);
    CuTexImage::CuTexObj srcTex =
        src->BindTexture(texDataBDesc, cudaCreateChannelDesc<float>());
    ConvertByteToFloat_Kernel<<<grid, block>>>(srcTex.handle,
                                               (float *)dst->_cuData);
}
...
void ProgramCU::FilterImage(CuTexImage *dst, CuTexImage *src, CuTexImage *buf) {
    ...
    /// vertical filtering
    CuTexImage::CuTexObj bufTex =
        buf->BindTexture(texDataDesc, cudaCreateChannelDesc<float>());
    ...
    FilterV<FW><<<gridv, blockv>>>(bufTex.handle, (float *)dst->_cuData, width,
                                   height);
...
void __global__ ComputeDOG_Kernel(cudaTextureObject_t texC,
                                  cudaTextureObject_t texP, float *d_dog,
                                  float2 *d_got, int width, int height) {
...
void ProgramCU::ComputeDOG(CuTexImage *gus, CuTexImage *dog, CuTexImage *got) {
    ...
    CuTexImage::CuTexObj texCObj =
        gus->BindTexture(texDataDesc, cudaCreateChannelDesc<float>());
    CuTexImage::CuTexObj texPObj =
        (gus - 1)->BindTexture(texDataDesc, cudaCreateChannelDesc<float>());
    if (got->_cuData)
        ComputeDOG_Kernel<<<grid, block>>>(
            texCObj.handle, texPObj.handle, (float *)dog->_cuData,
            (float2 *)got->_cuData, width, height);
    else
        ComputeDOG_Kernel<<<grid, block>>>(texCObj.handle, texPObj.handle,
                                           (float *)dog->_cuData, width,
                                           height);
}
...
void __global__ ComputeKEY_Kernel(cudaTextureObject_t texP,
                                  cudaTextureObject_t texC,
                                  cudaTextureObject_t texN, float4 *d_key,
                                  int width, int colmax, int rowmax,
                                  float dog_threshold0, float dog_threshold,
...
void ProgramCU::ComputeKEY(CuTexImage *dog, CuTexImage *key, float Tdog,
                           float Tedge) {
    ...
    CuTexImage::CuTexObj texPObj =
        dogp->BindTexture(texDataDesc, cudaCreateChannelDesc<float>());
    CuTexImage::CuTexObj texCObj =
        dog->BindTexture(texDataDesc, cudaCreateChannelDesc<float>());
    CuTexImage::CuTexObj texNObj =
        dogn->BindTexture(texDataDesc, cudaCreateChannelDesc<float>());
    Tedge = (Tedge + 1) * (Tedge + 1) / Tedge;
    ComputeKEY_Kernel<<<grid, block>>>(
        texPObj.handle, texCObj.handle, texNObj.handle, (float4 *)key->_cuData,
        width, width - 1, height - 1, Tdog1, Tdog, Tedge,
        GlobalUtil::_SubpixelLocalization);
}

void __global__ InitHist_Kernel(cudaTextureObject_t texDataF4, int4 *hist,
                                int ws, int wd, int height) {
...
void ProgramCU::InitHistogram(CuTexImage *key, CuTexImage *hist) {
    ...
    CuTexImage::CuTexObj keyTex =
        key->BindTexture(texDataDesc, cudaCreateChannelDesc<float4>());
    InitHist_Kernel<<<grid, block>>>(keyTex.handle, (int4 *)hist->_cuData, ws,
                                     wd, hd);
}

void __global__ ReduceHist_Kernel(cudaTextureObject_t texDataI4, int4 *d_hist,
                                  int ws, int wd, int height) {
    int row = IMUL(blockIdx.y, blockDim.y) + threadIdx.y;
    int col = IMUL(blockIdx.x, blockDim.x) + threadIdx.x;
    if (row < height && col < wd) {
...
void ProgramCU::ReduceHistogram(CuTexImage *hist1, CuTexImage *hist2) {
    ...
    CuTexImage::CuTexObj hist1Tex =
        hist1->BindTexture(texDataDesc, cudaCreateChannelDesc<int4>());

    const int BW = 1 << wi, BH = 1 << (7 - wi);
    dim3 grid((wd + BW - 1) / BW, (hd + BH - 1) / BH);
    dim3 block(BW, BH);
    ReduceHist_Kernel<<<grid, block>>>(hist1Tex.handle, (int4 *)hist2->_cuData,
                                       ws, wd, hd);
}

void __global__ ListGen_Kernel(cudaTextureObject_t texDataList,
                               cudaTextureObject_t texDataI4, int4 *d_list,
                               int list_len, int width) {
...
void ProgramCU::GenerateList(CuTexImage *list, CuTexImage *hist) {
    ...
    CuTexImage::CuTexObj listTex =
        list->BindTexture(texDataDesc, cudaCreateChannelDesc<int4>());
    CuTexImage::CuTexObj histTex =
        hist->BindTexture(texDataDesc, cudaCreateChannelDesc<int4>());
    dim3 grid((len + LISTGEN_BLOCK_DIM - 1) / LISTGEN_BLOCK_DIM);
    dim3 block(LISTGEN_BLOCK_DIM);
    ListGen_Kernel<<<grid, block>>>(listTex.handle, histTex.handle,
                                    (int4 *)list->_cuData, len,
                                    hist->GetImgWidth());
}

void __global__ ComputeOrientation_Kernel(
    cudaTextureObject_t texDataF2, cudaTextureObject_t texDataF4,
    cudaTextureObject_t texDataList, float4 *d_list, int list_len, int width,
    int height, float sigma, float sigma_step, float gaussian_factor,
    float sample_factor, int num_orientation, int existing_keypoint,
    int subpixel, int keepsign) {
    ...
    if (existing_keypoint) {
        ...
        if (subpixel || keepsign) {
            float4 offset =
                tex1Dfetch<float4>(texDataF4, IMUL(width, ikey.y) + ikey.x);
            if (subpixel) {
                key.x += offset.y;
                key.y += offset.z;
...
void ProgramCU::ComputeOrientation(CuTexImage *list, CuTexImage *got,
                                   CuTexImage *key, float sigma,
                                   float sigma_step, int existing_keypoint) {
    int len = list->GetImgWidth();
    if (len <= 0)
        return;
    ...
    if (existing_keypoint) {
        texObjF4 =
            list->BindTexture(texDataDesc, cudaCreateChannelDesc<float4>());
    } else {
        texObjList =
            list->BindTexture(texDataDesc, cudaCreateChannelDesc<int4>());
    ...
    CuTexImage::CuTexObj gotTex =
        got->BindTexture2D(texDataDesc, cudaCreateChannelDesc<float2>());
    ...
    ComputeOrientation_Kernel<<<grid, block>>>(
        gotTex.handle, texObjF4.handle, texObjList.handle,
...
void __global__ ComputeDescriptor_Kernel(cudaTextureObject_t texDataF2,
                                         cudaTextureObject_t texDataF4,
                                         float4 *d_des, int num, int width,
                                         int height, float window_factor) {
...
template <bool DYNAMIC_INDEXING>
void __global__ ComputeDescriptorRECT_Kernel(cudaTextureObject_t texDataF2,
                                             cudaTextureObject_t texDataF4,
                                             float4 *d_des, int num, int width,
                                             int height, float window_factor) {
    const float rpi = 4.0 / 3.14159265358979323846;
...
void __global__ NormalizeDescriptor_Kernel(cudaTextureObject_t texDataF4,
                                           float4 *d_des, int num) {
    float4 temp[32];
    int idx = IMUL(blockIdx.x, blockDim.x) + threadIdx.x;
    if (idx >= num)
        return;
...
void ProgramCU::ComputeDescriptor(CuTexImage *list, CuTexImage *got,
                                  CuTexImage *dtex, int rect, int stream) {
    ...
    if (rect) {
        if (GlobalUtil::_UseDynamicIndexing)
            ComputeDescriptorRECT_Kernel<true><<<grid, block>>>(
                gotTex.handle, listTex.handle, (float4 *)dtex->_cuData, num,
                width, height, GlobalUtil::_DescriptorWindowFactor);
        else
    ...
    if (GlobalUtil::_NormalizedSIFT) {
        CuTexImage::CuTexObj dtexTex =
            dtex->BindTexture(texDataDesc, cudaCreateChannelDesc<float4>());
        const int block_width = DESCRIPTOR_NORMALIZ_PER_BLOCK;
        dim3 grid((num + block_width - 1) / block_width);
        dim3 block(block_width);
        NormalizeDescriptor_Kernel<<<grid, block>>>(
            dtexTex.handle, (float4 *)dtex->_cuData, num);
    }
...
void __global__ ConvertDOG_Kernel(cudaTextureObject_t texData, float *d_result,
                                  int width, int height) {
...
void ProgramCU::DisplayConvertDOG(CuTexImage *dog, CuTexImage *out) {
    if (out->_cuData == NULL)
        return;
    int width = dog->GetImgWidth(), height = dog->GetImgHeight();
    CuTexImage::CuTexObj dogTex =
        dog->BindTexture(texDataDesc, cudaCreateChannelDesc<float>());
    dim3 grid((width + BLOCK_DIM - 1) / BLOCK_DIM,
              (height + BLOCK_DIM - 1) / BLOCK_DIM);
    dim3 block(BLOCK_DIM, BLOCK_DIM);
    ConvertDOG_Kernel<<<grid, block>>>(dogTex.handle, (float *)out->_cuData,
                                       width, height);
    ProgramCU::CheckErrorCUDA("DisplayConvertDOG");
}

void __global__ ConvertGRD_Kernel(cudaTextureObject_t texData, float *d_result,
                                  int width, int height) {
    int row = (blockIdx.y << BLOCK_LOG_DIM) + threadIdx.y;
    int col = (blockIdx.x << BLOCK_LOG_DIM) + threadIdx.x;
    if (col < width && row < height) {
...
void ProgramCU::DisplayConvertGRD(CuTexImage *got, CuTexImage *out) {
    ...
    int width = got->GetImgWidth(), height = got->GetImgHeight();
    CuTexImage::CuTexObj gotTex =
        got->BindTexture(texDataDesc, cudaCreateChannelDesc<float>());
    dim3 grid((width + BLOCK_DIM - 1) / BLOCK_DIM,
              (height + BLOCK_DIM - 1) / BLOCK_DIM);
    dim3 block(BLOCK_DIM, BLOCK_DIM);
    ConvertGRD_Kernel<<<grid, block>>>(gotTex.handle, (float *)out->_cuData,
                                       width, height);
    ProgramCU::CheckErrorCUDA("DisplayConvertGRD");
}

void __global__ ConvertKEY_Kernel(cudaTextureObject_t texData,
                                  cudaTextureObject_t texDataF4,
                                  float4 *d_result, int width, int height) {
    ...
    if (col < width && row < height) {
        ...
        float v =
            inside ? __saturatef(0.5 + 20 * tex1Dfetch<float>(texData, index))
                   : 0.5;
        d_result[index] =
            is_key && inside
...
void ProgramCU::DisplayConvertKEY(CuTexImage *key, CuTexImage *dog,
                                  CuTexImage *out) {
    if (out->_cuData == NULL)
        return;
    ...
    dim3 grid((width + BLOCK_DIM - 1) / BLOCK_DIM,
              (height + BLOCK_DIM - 1) / BLOCK_DIM);
    dim3 block(BLOCK_DIM, BLOCK_DIM);
    ConvertKEY_Kernel<<<grid, block>>>(dogTex.handle, keyTex.handle,
                                       (float4 *)out->_cuData, width, height);
}

void __global__ DisplayKeyPoint_Kernel(cudaTextureObject_t texDataF4,
                                       float4 *d_result, int num) {
    int idx = IMUL(blockIdx.x, blockDim.x) + threadIdx.x;
    if (idx >= num)
        return;
...
void ProgramCU::DisplayKeyPoint(CuTexImage *ftex, CuTexImage *out) {
    ...
    dim3 grid((num + block_width - 1) / block_width);
    dim3 block(block_width);
    CuTexImage::CuTexObj ftexTex =
        ftex->BindTexture(texDataDesc, cudaCreateChannelDesc<float4>());
    DisplayKeyPoint_Kernel<<<grid, block>>>(ftexTex.handle,
                                            (float4 *)out->_cuData, num);
    ProgramCU::CheckErrorCUDA("DisplayKeyPoint");
}

void __global__ DisplayKeyBox_Kernel(cudaTextureObject_t texDataF4,
                                     float4 *d_result, int num) {
    int idx = IMUL(blockIdx.x, blockDim.x) + threadIdx.x;
    if (idx >= num)
        return;
...
void ProgramCU::DisplayKeyBox(CuTexImage *ftex, CuTexImage *out) {
    ...
    CuTexImage::CuTexObj ftexTex =
        ftex->BindTexture(texDataDesc, cudaCreateChannelDesc<float4>());
    DisplayKeyBox_Kernel<<<grid, block>>>(ftexTex.handle,
                                          (float4 *)out->_cuData, len * 10);
}
...
void __global__ MultiplyDescriptor_Kernel(cudaTextureObject_t texDes1,
                                          cudaTextureObject_t texDes2,
                                          int *d_result, int num1, int num2,
                                          int3 *d_temp) {
...
void ProgramCU::MultiplyDescriptor(CuTexImage *des1, CuTexImage *des2,
    ...
    CuTexImage::CuTexObj des1Tex =
        des1->BindTexture(texDataDesc, cudaCreateChannelDesc<uint4>());
    CuTexImage::CuTexObj des2Tex =
        des2->BindTexture(texDataDesc, cudaCreateChannelDesc<uint4>());

    MultiplyDescriptor_Kernel<<<grid, block>>>(
        des1Tex.handle, des2Tex.handle, (int *)texDot->_cuData, num1, num2,
...
void __global__ MultiplyDescriptorG_Kernel(
    cudaTextureObject_t texDes1, cudaTextureObject_t texDes2,
    cudaTextureObject_t texLoc1, cudaTextureObject_t texLoc2, int *d_result,
    int num1, int num2, int3 *d_temp, Matrix33 H, float hdistmax, Matrix33 F,
    float fdistmax) {
    int idx01 = (blockIdx.y * MULT_BLOCK_DIMY);
...
void ProgramCU::MultiplyDescriptorG(CuTexImage *des1, CuTexImage *des2,
    ...
    CuTexImage::CuTexObj loc1Tex =
        loc1->BindTexture(texDataDesc, cudaCreateChannelDesc<float>());
    CuTexImage::CuTexObj loc2Tex =
        loc2->BindTexture(texDataDesc, cudaCreateChannelDesc<float2>());
    CuTexImage::CuTexObj des1Tex =
        des1->BindTexture(texDataDesc, cudaCreateChannelDesc<uint4>());
    CuTexImage::CuTexObj des2Tex =
        des2->BindTexture(texDataDesc, cudaCreateChannelDesc<uint4>());
    MultiplyDescriptorG_Kernel<<<grid, block>>>(
        des1Tex.handle, des2Tex.handle, loc1Tex.handle, loc2Tex.handle,
