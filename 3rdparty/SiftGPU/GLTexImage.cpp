									...
									const Uint * pin, Uint * pout)
{
    ...
    case GL_RGBA:
        ...
        for (i = 0; i < hs; i++, line += linestep) {
            for (j = 0, p = line; j < ws; j++, p += step) {
                ...
                *po++ =
                    ((19595 * p[0] + 38470 * p[1] + 7471 * p[2] + 32768) >> 16);
            }
        }
        break;
    case GL_BGR:
    case GL_BGRA:
        ...
        for (i = 0; i < hs; i++, line += linestep) {
            for (j = 0, p = line; j < ws; j++, p += step) {
                *po++ =
                    ((7471 * p[0] + 38470 * p[1] + 19595 * p[2] + 32768) >> 16);
            }
        }
        break;
    default:
    ...
    if (GlobalUtil::_UseCUDA || GlobalUtil::_UseOpenCL) {
        ...
        if (!simple_format) {
            ...
            if (gl_type == GL_UNSIGNED_BYTE)
                DownSamplePixelDataI2F(
                    gl_format, width, height, 1 << _down_sampled,
                ...
                DownSamplePixelDataF(gl_format, width, height,
                                     1 << _down_sampled, (float *)data,
                                     _converted_data, skip);
        ...
        if (simple_format &&
            ...
            if (gl_type == GL_UNSIGNED_BYTE) {
                ...
                glTexImage2D(_texTarget, 0,
                             GL_LUMINANCE32F_ARB, // internal format changed
                             _imgWidth, _imgHeight, 0, GL_LUMINANCE, GL_FLOAT,
                             newdata);
                delete[] newdata;
            } else {
                // impossible
            ...
            if (gl_format == GL_LUMINANCE || gl_format == GL_LUMINANCE_ALPHA) {
                ...
                glTexImage2D(_texTarget, 0, GL_LUMINANCE32F_ARB, _imgWidth,
                             _imgHeight, 0, gl_format, gl_type, data);
                GlobalUtil::FitViewPort(1, 1); // this used to be necessary
            } else {
                // convert RGB 2 GRAY if needed
                glTexImage2D(_texTarget, 0, _iTexFormat, _imgWidth, _imgHeight,
                             0, gl_format, gl_type, data);
                if (ShaderMan::HaveShaderMan())
                    TexConvertRGB();
                else
                    _rgb_converted = 0; // In CUDA mode, the conversion will be
                                        // done by CUDA kernel
            }
        }
        UnbindTex();
...
int GLTexInput::LoadImageFile(char *imagepath, int &w, int &h) {
    ...
    if (fscanf(file, "%s %d %d %d", buf, &width, &height, &cn) < 4 ||
        cn > 255 || width < 0 || height < 0) {
        fclose(file);
...
void GLTexImage::SaveToASCII(const char *path) {
    ...
    FrameBufferObject fbo;
    AttachToFBO(0);
    glReadPixels(0, 0, GetImgWidth(), GetImgHeight(), GL_RGBA, GL_FLOAT,
                 &buf[0]);
    ofstream out(path);
