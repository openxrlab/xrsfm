...
class GLTexImage : public GlobalUtil {
  ...
  public:
    ...
    void CopyFromPBO(GLuint pbo, int width, int height,
  ...
  public:
    ...
    // int	IsTexTight(){return _texWidth == _drawWidth && _texHeight ==
    // _drawHeight;}
...
class GLTexInput : public GLTexImage {
  ...
  public:
    static int IsSimpleGlFormat(unsigned int gl_format, unsigned int gl_type) {
        // the formats there is a cpu code to convert rgb and downsample
        return (gl_format == GL_LUMINANCE || gl_format == GL_LUMINANCE_ALPHA ||
               ...
               (gl_type == GL_UNSIGNED_BYTE || gl_type == GL_FLOAT ||
                gl_type == GL_UNSIGNED_SHORT);
    }
// in vc6, template member function doesn't work
#if !defined(_MSC_VER) || _MSC_VER > 1200
    template <class Uint>
    static int DownSamplePixelDataI(unsigned int gl_format, int width,
                                    int height, int ds, const Uint *pin,
                                    Uint *pout);
    template <class Uint>
    static int DownSamplePixelDataI2F(unsigned int gl_format, int width,
                                      int height, int ds, const Uint *pin,
                                      float *pout, int skip = 0);
#endif
    static int DownSamplePixelDataF(unsigned int gl_format, int width,
  ...
  public:
    GLTexInput()
        : _down_sampled(0), _rgb_converted(0), _data_modified(0),
