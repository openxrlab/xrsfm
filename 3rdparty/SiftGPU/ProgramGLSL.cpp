...
//		class ProgramGLSL		A simple wrapper of GLSL
// programs 		class ShaderBagGLSL		GLSL shaders for SIFT
...
ProgramGLSL::ShaderObject::ShaderObject(int shadertype, const char *source,
                                        int filesource) {
...
int ProgramGLSL::ShaderObject::ReadShaderFile(const char *sourcefile,
                                              char *&code) {
    code = NULL;
    FILE *file;
    int len = 0;
...
void ProgramGLSL::ReLink() { glLinkProgram(_programID); }

int ProgramGLSL::IsNative() { return _linked; }
...
FilterGLSL::FilterGLSL(float sigma) {
    ...
    if (GlobalUtil::_MaxFilterWidth > 0 &&
        width > GlobalUtil::_MaxFilterWidth) {
        std::cout << "Filter size truncated from " << width << " to "
                  << GlobalUtil::_MaxFilterWidth << endl;
    ...
    if (GlobalUtil::_verbose && GlobalUtil::_timingL)
        std::cout << "Filter: sigma = " << sigma << ", size = " << width << "x"
                  << width << endl;
}
...
ProgramGPU *FilterGLSL::CreateFilterH(float kernel[], int width) {
    ...
    for (int i = 0; i < width; i++) {
        if (i == half_width) {
            ...
            out << "pos = gl_TexCoord[0].st + vec2(float(" << (i - half_width)
                << ") , 0);\n";
            out << "intensity+= " << kernel[i]
                << "*texture2DRect(tex, pos).r;\n";
        }
    }
...
ProgramGPU *FilterGLSL::CreateFilterV(float kernel[], int height) {
    ...
    for (int i = 0; i < height; i++) {
        ...
        if (i == half_height) {
            ...
            out << "pos = gl_TexCoord[0].st + vec2(0, float("
                << (i - half_height) << ") );\n";
...
ProgramGPU *FilterGLSL::CreateFilterHPK(float kernel[], int width) {
    ...
    int nhpixel =
        (halfwidth + 1) >> 1; // how many neighbour pixels need to be looked up
    ...
    for (i = 0; i < npixel; i++) {
        out << "coord = gl_TexCoord[0].xy + vec2(float(" << i - nhpixel
            << "),0);\n";
        out << "pc=texture2DRect(tex, coord);\n";
        if (GlobalUtil::_PreciseBorder)
            out << "if(coord.x < 0.0) pc = pc.rrbb;\n";
        ...
        if (weight[1] == 0.0) {
            out << "result += vec4(" << weight[2] << "," << weight[0] << ","
                << weight[2] << "," << weight[0] << ")*pc.grab;\n";
        } else {
            out << "result += vec4(" << weight[1] << ", " << weight[0] << ", "
                << weight[1] << ", " << weight[0] << ")*pc.rrbb;\n";
            out << "result += vec4(" << weight[2] << ", " << weight[1] << ", "
                << weight[2] << ", " << weight[1] << ")*pc.ggaa;\n";
        }
    }
...
ProgramGPU *FilterGLSL::CreateFilterVPK(float kernel[], int height) {
    ...
    int nhpixel =
        (halfh + 1) >> 1; // how many neighbour pixels need to be looked up
    int npixel = (nhpixel << 1) + 1; //
    float weight[3];
    ostringstream out;
    ;
    ...
    for (i = 0; i < npixel; i++) {
        ...
        if (weight[1] == 0.0) {
            out << "result += vec4(" << weight[2] << "," << weight[2] << ","
                << weight[0] << "," << weight[0] << ")*pc.barg;\n";
        } else {
            out << "result += vec4(" << weight[1] << "," << weight[1] << ","
                << weight[0] << "," << weight[0] << ")*pc.rgrg;\n";
            out << "result += vec4(" << weight[2] << "," << weight[2] << ","
                << weight[1] << "," << weight[1] << ")*pc.baba;\n";
        }
    }
    out << "gl_FragColor = result;}\n" << '\0';
...
void ShaderBagGLSL::LoadFixedShaders() {
    ...
    s_gray = new ProgramGLSL(
        ...
        "float intensity = dot(vec3(0.299, 0.587, 0.114), texture2DRect(tex, "
    ...
    s_sampling = new ProgramGLSL(
        "uniform sampler2DRect tex; void main(void){gl_FragColor.rg= "
        "texture2DRect(tex, gl_TexCoord[0].st).rg;}");
    ...
    ProgramGLSL *program;
    s_margin_copy = program =
        new ProgramGLSL("uniform sampler2DRect tex; uniform vec2 truncate;\n"
                        "void main(){ gl_FragColor = texture2DRect(tex, "
                        "min(gl_TexCoord[0].xy, truncate)); }");
...
void ShaderBagGLSL::LoadDisplayShaders() {
    s_copy_key = new ProgramGLSL(
        "uniform sampler2DRect tex; void main(){\n"
        "gl_FragColor.rg= texture2DRect(tex, gl_TexCoord[0].st).rg; "
    ...
    s_vertex_list = program = new ProgramGLSL(
        ...
        "void main(void){\n"
        "float fwidth = sizes.y; float twidth = sizes.z; float rwidth = "
        "sizes.w; \n"
        "float index = 0.1*(fwidth*floor(gl_TexCoord[0].y) + "
        "gl_TexCoord[0].x);\n"
        ...
        "vec2 dxy = vec2(0); \n"
        "dxy.x = type < 0.1 ? 0.0 : (((type <0.5) || (type > 0.9))? size : "
        "-size);\n"
        "dxy.y = type < 0.2 ? 0.0 : (((type < 0.3) || (type > 0.7) )? -size "
        ":size); \n"
    ...
    s_display_gaussian =
        new ProgramGLSL("uniform sampler2DRect tex; void main(void){float r = "
                        "texture2DRect(tex, gl_TexCoord[0].st).r;\n"
                        "gl_FragColor = vec4(r, r, r, 1);}");

    s_display_dog =
        new ProgramGLSL("uniform sampler2DRect tex; void main(void){float g = "
                        "0.5+(20.0*texture2DRect(tex, gl_TexCoord[0].st).g);\n"
                        "gl_FragColor = vec4(g, g, g, 0.0);}");

    s_display_grad = new ProgramGLSL(
        "uniform sampler2DRect tex; void main(void){\n"
        "	vec4 cc = texture2DRect(tex, gl_TexCoord[0].st);gl_FragColor = "
        "vec4(5.0* cc.bbb, 1.0);}");

    s_display_keys = new ProgramGLSL(
        "uniform sampler2DRect tex; void main(void){\n"
        "	vec4 cc = texture2DRect(tex, gl_TexCoord[0].st);\n"
        "	if(cc.r ==0.0) discard; gl_FragColor =  (cc.r==1.0? vec4(1.0, "
        "0.0, 0,1.0):vec4(0.0,1.0,0.0,1.0));}");
}

void ShaderBagGLSL::LoadKeypointShader(float threshold, float edge_threshold) {
    float threshold0 =
        threshold * (GlobalUtil::_SubpixelLocalization ? 0.8f : 1.0f);
    ...
    out << "uniform sampler2DRect tex, texU, texD; void main ()\n"
        ...
        << "	float dog = 0.0; \n"
           ...
           "	dog = cc.g > float(THRESHOLD0) && all(greaterThan(cc.gggg, "
           "max(v1, v2)))?1.0: 0.0;\n"
           "	dog = cc.g < float(-THRESHOLD0) && all(lessThan(cc.gggg, "
           "min(v1, v2)))?0.5: dog;\n"
           "	if(dog == 0.0) return;\n";

    pos = out.tellp();
    // do edge suppression first..
    // vector v1 is < (-1, 0), (1, 0), (0,-1), (0, 1)>
    // vector v2 is < (-1,-1), (-1,1), (1,-1), (1, 1)>

    out << "	float fxx, fyy, fxy; \n"
           ...
           "	if( score_down <= 0.0 || score_up > THRESHOLD2 * "
           "score_down)return;\n";
    ...
    out << " \n"
        ...
        << "	if(dog == 1.0)\n"
           "	{\n"
           "		if(cc.g < v3.x || any(lessThan(cc.gggg, v4)) "
           "||any(lessThan(cc.gggg, v6)))return; \n"
           ...
           "		if(cc.g < v3.y || any(lessThan(cc.gggg, v5)) "
           "||any(lessThan(cc.gggg, v6)))return; \n"
           "	}\n"
        // the minimum case
        << "	else{\n"
           "	if(cc.g > v3.x || any(greaterThan(cc.gggg, v4)) "
           "||any(greaterThan(cc.gggg, v6)))return; \n"
           "		v3.y = texture2DRect(texD, gl_TexCoord[0].xy).g;\n"
           "		v5.x = texture2DRect(texD, gl_TexCoord[1].xy).g;\n"
           "		v5.y = texture2DRect(texD, gl_TexCoord[2].xy).g;\n"
           "		v5.z = texture2DRect(texD, gl_TexCoord[3].xy).g;\n"
           ...
           "		v6.z = texture2DRect(texD, gl_TexCoord[7].xy).g;\n"
           "		v6.w = texture2DRect(texD, TexRU.xy).g;\n"
           "		if(cc.g > v3.y || any(greaterThan(cc.gggg, v5)) "
           "||any(greaterThan(cc.gggg, v6)))return; \n"
           "	}\n";
    ...
    if (GlobalUtil::_SubpixelLocalization)
        ...
        out << "	fs = 0.5*( v3.x - v3.y );  \n"
            ...
            << "	vec3 dxys = vec3(0.0);		\n"
               ...
               "		{					"
               "						\n"
               "			vec4 TEMP = A1; A1 = A0; A0 = "
               "TEMP;	\n"
               "		}else if( x3.z == maxa )		"
               "			\n"
               "		{					"
               "						\n"
               "			vec4 TEMP = A2; A2 = A0; A0 = "
               "TEMP;	\n"
               "		}					"
               "						\n"
               "		A0 /= A0.x;				"
            ...
            << "				\n"
               ...
               "				dxys.y = A1.w - "
               "dxys.z*A1.z;			    \n"
               "				dxys.x = A0.w - dxys.z*A0.z - "
               "dxys.y*A0.y;	\n"

            // one more threshold which I forgot in versions prior to 286
            << "				bool dog_test = (abs(cc.g + "
               "0.5*dot(vec3(fx, fy, fs), dxys ))<= float(THRESHOLD1)) ;\n"
               "				if(dog_test || "
               "any(greaterThan(abs(dxys), vec3(1.0)))) dog = 0.0;\n"
               "			}\n"
               "		}\n"
               "	}\n"
        ...
        out << "	gl_FragData[1] =  vec4( dog, 0.0, 0.0, 0.0) ;	\n";
...
void ShaderBagGLSL::UnloadProgram() { glUseProgram(0); }
...
void ShaderBagGLSL::LoadGenListShader(int ndoglev, int nlev) {
    ...
    s_genlist_init_tight = new ProgramGLSL(
        "uniform sampler2DRect tex; void main (void){\n"
        "vec4 helper = vec4( texture2DRect(tex, gl_TexCoord[0].xy).r,  "
        "texture2DRect(tex, gl_TexCoord[1].xy).r,\n"
        "texture2DRect(tex, gl_TexCoord[2].xy).r, texture2DRect(tex, "
        "gl_TexCoord[3].xy).r);\n"
    ...
    s_genlist_histo =
        new ProgramGLSL("uniform sampler2DRect tex; void main (void){\n"
                        ...
                        "helper = texture2DRect(tex, gl_TexCoord[0].xy); "
                        "helper2.xy = helper.xy + helper.zw; \n"
                        "helper = texture2DRect(tex, gl_TexCoord[1].xy); "
                        "helper2.zw = helper.xy + helper.zw; \n"
                        "gl_FragColor.rg = helper2.xz + helper2.yw;\n"
                        "helper = texture2DRect(tex, gl_TexCoord[2].xy); "
                        "helper2.xy = helper.xy + helper.zw; \n"
                        "helper = texture2DRect(tex, gl_TexCoord[3].xy); "
                        "helper2.zw = helper.xy + helper.zw; \n"
...
ProgramGLSL *ShaderBagGLSL::LoadGenListStepShader(int start, int step) {
    ...
    if (start) {
        ...
        out << "void main(void){\n";
        out << "float  index = floor(gl_TexCoord[0].y) * width + "
               "floor(gl_TexCoord[0].x);\n";
    ...
    if (step > 0) {
        ...
        for (i = 0; i < step; i++) {
            ...
            out << "else if(index < sum.y ) {opos = cpos.xy; index -= "
                   "sum.x;}\n";
...
void ShaderBagGLSL::LoadOrientationShader() {
    ...
    out << "\n"
        ...
        << "void main()		\n"
           ...
           "	vec4 bins[10];						"
           "		\n"
           "	bins[0] = vec4(0.0);bins[1] = vec4(0.0);bins[2] = "
           "vec4(0.0);	\n"
           "	bins[3] = vec4(0.0);bins[4] = vec4(0.0);bins[5] = "
           "vec4(0.0);	\n"
           "	bins[6] = vec4(0.0);bins[7] = vec4(0.0);bins[8] = "
           "vec4(0.0);	\n"
    ...
    out << "	//bool fixed_orientation = (size.z < 0.0);		\n"
           "	if(size.z < 0.0) {gl_FragData[0] = vec4(pos, 0.0, sigma); "
           "return;}"
    ...
    out << "\n"
           ...
           "					\n"
           "				vec4 cc = texture2DRect(gradTex, "
           "spos);				\n"
           "				float grad = cc.b;	float theta = "
           "cc.a;				\n"
           "				float idx = "
           "floor(degrees(theta)*0.1);				\n"
           "				if(idx < 0.0 ) idx += 36.0;	"
           "								\n"
           "				float weight = grad*exp(sq_dist * "
           "factor);				\n"
           "				float vidx = fract(idx * 0.25) * "
           "4.0;//mod(idx, 4.0) ;					"
           "		\n"
           "				vec4 inc = "
           "weight*vec4(equal(vec4(vidx), vec4(0.0,1.0,2.0,3.0)));";
...
void ShaderBagGLSL::WriteOrientationCodeToStream(std::ostream &out) {
    ...
    */
    ...
    out << "\n"
           ...
           "	mat4 m2 = mat4(7, 6, 3, 1, 6, 7, 6, 3, 3, 6, 7, 6, 1, 3, 6, "
           "7)/27.0;\n"
    ...
    out << "\n"
           ...
           "	vec4 maxh4 = max(max(max(max(max(max(max(max(bins[0], "
           "bins[1]), bins[2]), \n"
           "			bins[3]), bins[4]), bins[5]), bins[6]), "
           "bins[7]), bins[8]);\n"
           "	maxh2 = max(maxh4.xy, maxh4.zw); maxh = vec4(max(maxh2.x, "
           "maxh2.y));";
    ...
    if (GlobalUtil::_MaxOrientation > 1) {
        ...
        if (GlobalUtil::_MaxOrientation == 2) {
            savepeak_code =
                ...
                "			{weights.rg = vec2(weight, weights.r); "
                "Orientations.rg = vec2(th, Orientations.r);}\\\n"
                "			else {weights.g = weight; "
                "Orientations.g = th;}";
            ...
            savepeak_code =
                ...
                "			if(weight <=weights.b){}\\\n"
                "			else if(weight >weights.r)\\\n"
                "			{weights.rgb = vec3(weight, "
                "weights.rg); Orientations.rgb = vec3(th, "
                "Orientations.rg);}\\\n"
                "			else if(weight >weights.g)\\\n"
                "			{weights.gb = vec2(weight, weights.g); "
                "Orientations.gb = vec2(th, Orientations.g);}\\\n"
                "			else {weights.b = weight; "
                "Orientations.b = th;}";
        } else {
            savepeak_code =
                "\\\n"
                "			if(weight <=weights.a){}\\\n"
                "			else if(weight >weights.r)\\\n"
                "			{weights = vec4(weight, weights.rgb); "
                "Orientations = vec4(th, Orientations.rgb);}\\\n"
                "			else if(weight >weights.g)\\\n"
                "			{weights.gba = vec3(weight, "
                "weights.gb); Orientations.gba = vec3(th, "
                "Orientations.gb);}\\\n"
                "			else if(weight >weights.b)\\\n"
                "			{weights.ba = vec2(weight, weights.b); "
                "Orientations.ba = vec2(th, Orientations.b);}\\\n"
                "			else {weights.a = weight; "
                "Orientations.a = th;}";
        }
    ...
    out << "\n"
        ...
        << "\\\n"
           ...
           "	{							"
           "				\\\n"
           "		if(test.r && bins[i].x > prevb && bins[i].x > "
           "bins[i].y )	\\\n"
           "		{						"
           "					\\\n"
           "		    float	di = -0.5 * (bins[i].y-prevb) / "
           "(bins[i].y+prevb-bins[i].x - bins[i].x) ; \\\n"
           "		    float	th = (k+di+0.5);	float weight = "
           "bins[i].x;"
        << savepeak_code
        << "\\\n"
           "		}\\\n"
           "		else if(test.g && all( greaterThan(bins[i].yy , "
           "bins[i].xz)) )	\\\n"
           "		{						"
           "					\\\n"
           "		    float	di = -0.5 * (bins[i].z-bins[i].x) / "
           "(bins[i].z+bins[i].x-bins[i].y- bins[i].y) ; \\\n"
           "		    float	th = (k+di+1.5);	float weight = "
           "bins[i].y;				"
        << savepeak_code
        << "	\\\n"
           "		}\\\n"
           "		if(test.b && all( greaterThan( bins[i].zz , "
           "bins[i].yw)) )	\\\n"
           "		{						"
           "					\\\n"
           "		    float	di = -0.5 * (bins[i].w-bins[i].y) / "
           "(bins[i].w+bins[i].y-bins[i].z- bins[i].z) ; \\\n"
           "		    float	th = (k+di+2.5);	float weight = "
           "bins[i].z;				"
        << savepeak_code
        << "	\\\n"
           "		}\\\n"
           "		else if(test.a && bins[i].w > bins[i].z && bins[i].w > "
           "bins[i+1].x )	\\\n"
           "		{						"
           "					\\\n"
           "		    float	di = -0.5 * (bins[i+1].x-bins[i].z) / "
           "(bins[i+1].x+bins[i].z-bins[i].w - bins[i].w) ; \\\n"
           "		    float	th = (k+di+3.5);	float weight = "
           "bins[i].w;				"
        << savepeak_code
        << "	\\\n"
           "		}\\\n"
    ...
    if (GlobalUtil::_KeepShaderLoop) {
        out << "\n"
               "	vec4 hh = maxh * ORIENTATION_THRESHOLD;	bvec4 "
               "test;	\n"
               "	bins[9] = bins[0];				"
               "				\n"
    ...
    if (GlobalUtil::_MaxOrientation > 1) {
        out << "\n"
            ...
            << (GlobalUtil::_MaxOrientation > 3 ? 1 : 0)
            << "), vec4(greaterThan(weights, hh)));\n"
               "		gl_FragData[0] = vec4(pos, npeaks, sigma);\n"
               "		gl_FragData[1] = radians((Orientations "
               ")*10.0);\n"
               "	}else{\n"
               "		gl_FragData[0] = vec4(pos, "
               "radians((Orientations.x)*10.0), sigma);\n"
               "	}\n";
    } else {
        out << "\n"
               "	 gl_FragData[0] = vec4(pos, "
               "radians((Orientation)*10.0), sigma);\n";
    }
    // end
    out << "\n"
           "}\n"
...
void ShaderBagGLSL::SetSimpleOrientationInput(int oTex, float sigma,
                                              float sigma_step) {
    glUniform1i(_param_orientation_gtex, 1);
    glUniform1f(_param_orientation_size, sigma);
}

void ShaderBagGLSL::SetFeatureOrientationParam(int gtex, int width, int height,
                                               float sigma, int stex,
                                               float step) {
    ///
    glUniform1i(_param_orientation_gtex, 1);
...
void ShaderBagGLSL::LoadDescriptorShaderF2() {
    ...
    out << "\n"
           ...
           "	\n"
           "	float index = dsize.x*floor(gl_TexCoord[0].y * 0.5) + "
           "gl_TexCoord[0].x;\n"
           "	float idx = 8.0 * fract(index * 0.125) + 8.0 * floor(2.0 * "
           "fract(gl_TexCoord[0].y * 0.5));		\n"
           "	index = floor(index*0.125) + 0.49;  \n"
           "	vec2 coord = floor( vec2( mod(index, dsize.z), index*dsize.w)) "
           "+ 0.5 ;\n"
           "	vec2 pos = texture2DRect(tex, coord).xy;		\n"
           "	if(any(lessThanEqual(pos.xy,  vec2(1.0))) || "
           "any(greaterThanEqual(pos.xy, dim-1.0)))// discard;	\n"
    ...
    // devide the 4x4 sift grid into 16 1x1 block, and each corresponds to a
    // shader thread To use linear interoplation, 1x1 is increased to 2x2, by
    // adding 0.5 to each side

    out << "vec4 temp; vec2 pt, offsetpt;				\n"
    ...
    out << "\n"
           ...
           "				vec4 cc = texture2DRect(gradTex, "
           "spos);						\n"
           "				float mod = cc.b;	float angle = "
           "cc.a;					\n"
           "				float theta0 = RPI * (anglef - "
           "angle);				\n"
           "				float theta = theta0 < 0.0? theta0 + "
           "8.0 : theta0;;\n"
           "				diff = nxy + offsetpt.xy;	"
           "							\n"
           "				float ww = exp(-0.125*dot(diff, "
           "diff));\n"
           "				vec2 weights = vec2(1) - nxyn;\n"
           "				float weight = weights.x * weights.y "
           "*mod*ww; \n"
           "				float theta1 = floor(theta); \n"
           "				float weight2 = (theta - theta1) * "
           "weight;\n"
           "				float weight1 = weight - weight2;\n"
           "				DA += vec4(equal(vec4(theta1),  "
           "vec4(0, 1, 2, 3)))*weight1;\n"
           "				DA += vec4(equal(vec4(theta1),  "
           "vec4(7, 0, 1, 2)))*weight2; \n"
           "				DB += vec4(equal(vec4(theta1),  "
           "vec4(4, 5, 6, 7)))*weight1;\n"
           "				DB += vec4(equal(vec4(theta1),  "
           "vec4(3, 4, 5, 6)))*weight2; \n"
           "			}\n"
           "		}\n"
           "	}\n";
...
void ShaderBagGLSL::SetFeatureDescirptorParam(int gtex, int otex, float dwidth,
                                              float fwidth, float width,
                                              float height, float sigma) {
    ///
...
void ShaderBagPKSL::LoadFixedShaders() {
    ...
    s_gray = new ProgramGLSL(
        ...
        "texture2DRect(tex,gl_TexCoord[0].xy ).rgb);\n"
        "gl_FragColor= vec4(intensity, intensity, intensity, 1.0);}");

    s_sampling = new ProgramGLSL(
        "uniform sampler2DRect tex; void main(){\n"
        "gl_FragColor= vec4(	texture2DRect(tex,gl_TexCoord[0].st "
        ").r,texture2DRect(tex,gl_TexCoord[1].st ).r,\n"
        "						"
        "texture2DRect(tex,gl_TexCoord[2].st "
        ").r,texture2DRect(tex,gl_TexCoord[3].st ).r);}");
    ...
    s_margin_copy = program = new ProgramGLSL(
        ...
        "\n"
        "else {	vec4 weights = vec4(vec4(0.0, 1.0, 2.0, 3.0) == "
        "truncate.wwww);\n"
    ...
    s_grad_pass = program = new ProgramGLSL(
        ...
        "	gl_FragData[0] = cc - cp; \n"
        "	vec4 cl = texture2DRect(tex, gl_TexCoord[1].xy); vec4 cr = "
        "texture2DRect(tex, gl_TexCoord[2].xy);\n"
        "	vec4 cd = texture2DRect(tex, gl_TexCoord[3].xy); vec4 cu = "
        "texture2DRect(tex, gl_TexCoord[4].xy);\n"
    ...
    if (s_orientation == NULL) {
        // Load a simplified version if the right version is not supported
        s_orientation = program = new ProgramGLSL(
            "uniform sampler2DRect tex; uniform sampler2DRect oTex; uniform "
            "vec2 size; void main(){\n"
            "	vec4 cc = texture2DRect(tex, gl_TexCoord[0].xy);\n"
            "	vec2 co = cc.xy * 0.5; \n"
...
void ShaderBagPKSL::LoadDisplayShaders() {
    ...
    s_vertex_list = program = new ProgramGLSL(
        ...
        "	float type = fract(px);\n"
        "	vec2 dxy; float s, c;\n"
        "	dxy.x = type < 0.1 ? 0.0 : (((type <0.5) || (type > 0.9))? "
        "size : -size);\n"
        "	dxy.y = type < 0.2 ? 0.0 : (((type < 0.3) || (type > 0.7) )? "
        "-size :size); \n"
    ...
    s_display_gaussian = new ProgramGLSL(
        "uniform sampler2DRect tex; void main(){\n"
        "vec4 pc = texture2DRect(tex, gl_TexCoord[0].xy);	bvec2 ff = "
        "lessThan(fract(gl_TexCoord[0].xy), vec2(0.5));\n"
        "float v = ff.y?(ff.x? pc.r : pc.g):(ff.x?pc.b:pc.a); gl_FragColor = "
        "vec4(vec3(v), 1.0);}");
    ...
    s_display_dog = new ProgramGLSL(
        ...
        "vec4 pc = texture2DRect(tex, gl_TexCoord[0].xy); bvec2 ff = "
        "lessThan(fract(gl_TexCoord[0].xy), vec2(0.5));\n"
        "float v = ff.y ?(ff.x ? pc.r : pc.g):(ff.x ? pc.b : pc.a);float g = "
        "(0.5+20.0*v);\n"
    ...
    s_display_grad = new ProgramGLSL(
        ...
        "vec4 pc = texture2DRect(tex, gl_TexCoord[0].xy); bvec2 ff = "
        "lessThan(fract(gl_TexCoord[0].xy), vec2(0.5));\n"
        "float v = ff.y ?(ff.x ? pc.r : pc.g):(ff.x ? pc.b : pc.a); "
        "gl_FragColor = vec4(5.0 *vec3(v), 1.0); }");
...
void ShaderBagPKSL::LoadOrientationShader(void) {
    ...
    out << "\n"
        ...
        << "\n"
           ...
           "	vec2 pos = sift.xy; \n"
           "	bool orientation_mode = (size.z != 0.0);		\n"
           "	float sigma = orientation_mode? (abs(size.z) * pow(size.w, "
           "sift.w) * sift.z) : (sift.w); \n"
           "	//bool fixed_orientation = (size.z < 0.0);		\n"
           "	if(size.z < 0.0) {gl_FragData[0] = vec4(pos, 0.0, sigma); "
           "return;}"
           "	float gsigma = sigma * GAUSSIAN_WF;			"
           "	\n"
           "	vec2 win = abs(vec2(sigma * (SAMPLE_WF * "
           "GAUSSIAN_WF)));	\n"
           "	vec2 dim = size.xy;					"
           "		\n"
    ...
    out << "\n"
           ...
           "								"
           "	\n"
           "			vec2 offset = 2.0 * spos - pos - "
           "vec2(0.5);					\n"
           "			vec4 off = vec4(offset, offset + "
           "vec2(1));				\n"
           "			vec4 distsq = off.xzxz * off.xzxz + off.yyww * "
           "off.yyww;	\n"
           "			bvec4 inside = lessThan(distsq, "
           "dist_threshold);			\n"
           "			if(any(inside))				"
           "						\n"
           "			{					"
           "								"
           "	\n"
           "				vec4 gg = texture2DRect(gtex, "
           "spos);				\n"
           "				vec4 oo = texture2DRect(otex, "
           "spos);				\n"
           "				vec4 weight = gg * exp(distsq * "
           "factor);			\n"
           "				vec4 idxv  = floor(degrees(oo)*0.1); "
           "				\n"
           "				idxv+= (vec4(lessThan(idxv, "
           "vec4(0.0)))*36.0); 			\n"
           "				vec4 vidx = fract(idxv * 0.25) * "
           "4.0;//mod(idxv, 4.0);	\n";
    //
    if (GlobalUtil::_UseDynamicIndexing) {
        ...
        out << "\n"
               ...
               "		\n"
               "						vec4 inc = "
               "weight[i] * vec4(equal(vec4(vidx[i]), "
        ...
        // nvfp40 still does not support dynamic array indexing
        // unrolled binary search
        // it seems to be faster than the dynamic indexing version on some GPUs
        out << "\n"
               "				for(int i = 0 ; i < 4; i++)\n"
               "				{\n"
               "					if(inside[i])\n"
               ...
               "				\n"
               "						vec4 inc = "
               "weight[i] * vec4(equal(vec4(vidx[i]), vec4(0,1,2,3)));	\n"
               "						if(idx < "
               "16.0)							\n"
               "						{	"
               "							"
...
void ShaderBagPKSL::LoadGenListShader(int ndoglev, int nlev) {
    ...
    s_genlist_init_tight = new ProgramGLSL(
        ...
        "gl_TexCoord[3].xy).r); \n"
        "					gl_FragColor = "
        "vec4(notEqual(key, vec4(0.0))); \n"
        "}");

    s_genlist_init_ex = program = new ProgramGLSL(
        "uniform sampler2DRect tex; uniform vec4 bbox; void main ()\n"
        "{\n"
        "	vec4 helper1 = vec4(equal(vec4(abs(texture2DRect(tex, "
        "gl_TexCoord[0].xy).r)), vec4(1.0, 2.0, 3.0, 4.0)));\n"
        "	vec4 helper2 = vec4(equal(vec4(abs(texture2DRect(tex, "
        "gl_TexCoord[1].xy).r)), vec4(1.0, 2.0, 3.0, 4.0)));\n"
        "	vec4 helper3 = vec4(equal(vec4(abs(texture2DRect(tex, "
        "gl_TexCoord[2].xy).r)), vec4(1.0, 2.0, 3.0, 4.0)));\n"
        "	vec4 helper4 = vec4(equal(vec4(abs(texture2DRect(tex, "
        "gl_TexCoord[3].xy).r)), vec4(1.0, 2.0, 3.0, 4.0)));\n"
        ...
        "	gl_FragColor.r = float(any(greaterThan(max(helper1.xy, "
        "helper1.zw), vec2(0.0))));	\n"
        "	gl_FragColor.g = float(any(greaterThan(max(helper2.xy, "
        "helper2.zw), vec2(0.0))));	\n"
        "	gl_FragColor.b = float(any(greaterThan(max(helper3.xy, "
        "helper3.zw), vec2(0.0))));	\n"
        "	gl_FragColor.a = float(any(greaterThan(max(helper4.xy, "
        "helper4.zw), vec2(0.0))));	\n"
        "}");
    _param_genlist_init_bbox = glGetUniformLocation(*program, "bbox");
    ...
    s_genlist_end = program = new ProgramGLSL(
        GlobalUtil::_KeepExtremumSign == 0
            ...
            "uniform sampler2DRect tex; uniform sampler2DRect ktex; void "
            "main()\n"
            "{\n"
            "	vec4 tc = texture2DRect( tex, gl_TexCoord[0].xy);\n"
            "	vec2 pos = tc.rg; float index = tc.b;\n"
            "	vec4 tk = texture2DRect( ktex, pos); \n"
            "	vec4 keys = vec4(equal(abs(tk.rrrr), vec4(1.0, 2.0, 3.0, "
            "4.0))); \n"
            "	vec2 opos; \n"
            ...
            "	opos.x = dot(keys, vec4(-0.5, 0.5, -0.5, 0.5));\n"
            "	opos.y = dot(keys, vec4(-0.5, -0.5, 0.5, 0.5));\n"
            "	gl_FragColor = vec4(opos + pos * 2.0 + tk.yz, sign(tk.r), "
            "tk.w);\n"
            "}");
    ...
    s_genlist_histo = new ProgramGLSL(
        ...
        "{\n"
        "	vec4 helper; vec4 helper2; \n"
        "	helper = texture2DRect(tex, gl_TexCoord[0].xy); helper2.xy = "
        "helper.xy + helper.zw; \n"
        "	helper = texture2DRect(tex, gl_TexCoord[1].xy); helper2.zw = "
        "helper.xy + helper.zw; \n"
        "	gl_FragColor.rg = helper2.xz + helper2.yw;\n"
        "	helper = texture2DRect(tex, gl_TexCoord[2].xy); helper2.xy = "
        "helper.xy + helper.zw; \n"
        "	helper = texture2DRect(tex, gl_TexCoord[3].xy); helper2.zw = "
        "helper.xy + helper.zw; \n"
        "	gl_FragColor.ba= helper2.xz+helper2.yw;\n"
        "}");
...
void ShaderBagPKSL::UnloadProgram(void) { glUseProgram(0); }
void ShaderBagPKSL::LoadKeypointShader(float dog_threshold,
                                       float edge_threshold) {
    float threshold0 =
        dog_threshold * (GlobalUtil::_SubpixelLocalization ? 0.8f : 1.0f);
    ...
    if (GlobalUtil::_DarknessAdaption) {
        out << "#define THRESHOLD0(i) (" << threshold0
            ...
            << threshold2
            << "\n"
               "#define DEFINE_EXTRA() vec4 ii = texture2DRect(texI, "
               "gl_TexCoord[0].xy); "
    ...
    out << "	vec4 key = vec4(0.0); \n"
           "	#define KEYTEST_STEP0(i) \\\n"
           "	{\\\n"
           "		bvec4 test1 = greaterThan(vec4(cc[i]), max(v1[i], "
           "v2[i])), test2 = lessThan(vec4(cc[i]), min(v1[i], v2[i]));\\\n"
           "		key[i] = cc[i] > float(THRESHOLD0(i)) && "
           "all(test1)?1.0: 0.0;\\\n"
           "		key[i] = cc[i] < float(-THRESHOLD0(i)) && all(test2)? "
           "-1.0: key[i];\\\n"
           "	}\n"
    ...
    out << "	float fxx[4], fyy[4], fxy[4], fx[4], fy[4];\n"
    ...
    out << "	vec4  v4[4], v5[4], v6[4];\n"
        ...
        << "	#define KEYTEST_STEP1(i)\\\n"
           ...
           "	}else if(key[i] == -1.0)\\\n"
           "	{\\\n"
           "		bvec4 test = greaterThan(vec4(cc[i]), min(v4[i], "
           "v6[i])); \\\n"
           "		if(cc[i] > cu[i] || any(test) )key[i] = 0.0; \\\n"
           "	}\n"
        ...
        << "	#define KEYTEST_STEP2(i)\\\n"
           "	if(key[i] == 1.0)\\\n"
           "	{\\\n"
           "		bvec4 test = lessThan(vec4(cc[i]), max(v5[i], "
           "v6[i]));\\\n"
           "		if(cc[i] < cd[i] || any(test))key[i] = 0.0; \\\n"
           "	}else if(key[i] == -1.0)\\\n"
           "	{\\\n"
           "		bvec4 test = greaterThan(vec4(cc[i]), min(v5[i], "
           "v6[i]));\\\n"
           "		if(cc[i] > cd[i] || any(test))key[i] = 0.0; \\\n"
           "	}\n"
    ...
    if (GlobalUtil::_SubpixelLocalization)
        ...
        out << "	vec3 offset = vec3(0.0, 0.0, 0.0); \n"
               ...
               "v5[idx];						\\\n"
               "		fxy[0] = fxy[idx];	fxx[0] = "
               "fxx[idx];	fyy[0] = fyy[idx];	\\\n"
               "		fx[0] = fx[idx];	fy[0] = "
               "fy[idx];	MOVE_EXTRA(idx);  \\\n"
               "	}\n"
               "	TESTMOVE_KEYPOINT(1);\n"
               "	TESTMOVE_KEYPOINT(2);\n"
               "	TESTMOVE_KEYPOINT(3);\n"
            ...
            "				offset.y = A1.w - "
            "offset.z*A1.z;			    \n"
            "				offset.x = A0.w - offset.z*A0.z - "
            "offset.y*A0.y;	\n"
            "				bool test = (abs(cc[0] + "
            "0.5*dot(vec3(fx[0], fy[0], fs), offset ))>float(THRESHOLD1)) ;\n"
            "				if(!test || any( "
            "greaterThan(abs(offset), vec3(1.0)))) key = vec4(0.0);\n"
            "			}\n"
            "		}\n"
            "	}\n"
            << "\n"
    ...
    _param_dog_texd = glGetUniformLocation(*program, "texD");
    if (GlobalUtil::_DarknessAdaption)
        _param_dog_texi = glGetUniformLocation(*program, "texI");
}
void ShaderBagPKSL::SetDogTexParam(int texU, int texD) {
    glUniform1i(_param_dog_texu, 1);
    glUniform1i(_param_dog_texd, 2);
    if (GlobalUtil::_DarknessAdaption)
        glUniform1i(_param_dog_texi, 3);
}
...
ProgramGLSL *ShaderBagPKSL::LoadDescriptorProgramRECT() {
    ...
    out << "\n"
           ...
           "					\n"
           "			bvec4 inside = lessThan(max(nxn, nyn) , "
           "vec4(1.0));	\n"
           "			if(any(inside))\n"
           "			{\n"
           ...
           "				vec4 theta0 = (- oo)*RPI;\n"
           "				vec4 theta = 8.0 * fract(1.0 + 0.125 * "
           "theta0);			\n"
           "				vec4 theta1 = floor(theta);	"
           "							\n"
           "				vec4 weight = (vec4(1) - nxn) * "
           "(vec4(1) - nyn) * gg; \n"
           "				vec4 weight2 = (theta - theta1) * "
           "weight;				\n"
           "				vec4 weight1 = weight - "
           "weight2;						\n"
           ...
           "				{\\\n"
           "					DA += "
           "vec4(equal(vec4(theta1[i]), vec4(0, 1, 2, 3)))*weight1[i]; \\\n"
           "					DA += "
           "vec4(equal(vec4(theta1[i]), vec4(7, 0, 1, 2)))*weight2[i]; \\\n"
           "					DB += "
           "vec4(equal(vec4(theta1[i]), vec4(4, 5, 6, 7)))*weight1[i]; \\\n"
           "					DB += "
           "vec4(equal(vec4(theta1[i]), vec4(3, 4, 5, 6)))*weight2[i]; \\\n"
           "				}\n"
...
ProgramGLSL *ShaderBagPKSL::LoadDescriptorProgramPKSL() {
    ...
    out << "\n"
           ...
           "+ 0.5 ;\n"
           "	vec2 pos = texture2DRect(tex, coord).xy;		\n"
           "	if(any(lessThan(pos.xy, vec2(1.0))) || any(greaterThan(pos.xy, "
           "dim-1.0))) "
           "	//discard;	\n"
           "	{ gl_FragData[0] = gl_FragData[1] = vec4(0.0); return; }\n"
           "	float anglef = texture2DRect(tex, coord).z;\n"
    ...
    out << "\n"
           ...
           "				vec4 theta1 = floor(theta);	"
           "							\n"
           "				vec4 diffx = nx + offsetpt.x, diffy = "
           "ny + offsetpt.y;	\n"
           "				vec4 ww = exp(-0.125 * (diffx * diffx "
           "+ diffy * diffy ));	\n"
           "				vec4 weight = (vec4(1) - nxn) * "
           "(vec4(1) - nyn) * gg * ww; \n"
           "				vec4 weight2 = (theta - theta1) * "
...
void ShaderBagPKSL::SetSimpleOrientationInput(int oTex, float sigma,
                                              float sigma_step) {
    glUniform1i(_param_orientation_gtex, 1);
    glUniform2f(_param_orientation_size, sigma, sigma_step);
}

void ShaderBagPKSL::SetFeatureOrientationParam(int gtex, int width, int height,
                                               float sigma, int otex,
                                               float step) {
    ///
    glUniform1i(_param_orientation_gtex, 1);
...
void ShaderBagPKSL::SetFeatureDescirptorParam(int gtex, int otex, float dwidth,
                                              float fwidth, float width,
                                              float height, float sigma) {
    if (sigma == 0 && s_rect_description) {
        // rectangle description mode
        s_rect_description->UseProgram();
        GLint param_descriptor_gtex =
            glGetUniformLocation(*s_rect_description, "gtex");
        GLint param_descriptor_otex =
            glGetUniformLocation(*s_rect_description, "otex");
        GLint param_descriptor_size =
            glGetUniformLocation(*s_rect_description, "size");
        GLint param_descriptor_dsize =
            glGetUniformLocation(*s_rect_description, "dsize");
        ///
...
void ShaderBagPKSL::SetGenListInitParam(int w, int h) {
    float bbox[4] = {(w - 1.0f) * 0.5f + 0.25f, (w - 1.0f) * 0.5f - 0.25f,
                     (h - 1.0f) * 0.5f + 0.25f, (h - 1.0f) * 0.5f - 0.25f};
