 ...
 * @param R In/Outparam. Should be set to initial guess at R. Will be modified
 * to be the optimal translation.
 ...
 */
double orthogonal_iteration(matd_t **v, matd_t **p, matd_t **t, matd_t **R,
                            int n_points, int n_steps) {
    ...
    *n_roots = 0;
    for (int i = 0; i <= n_der_roots; i++) {
        ...
        if (polyval(p, degree, min) * polyval(p, degree, max) < 0) {
            // We have a zero-crossing in this interval, use a combination of
            // Newton' and bisection. Some thanks to Numerical Recipes in C.
 ...
 */
matd_t *fix_pose_ambiguities(matd_t **v, matd_t **p, matd_t *t, matd_t *R,
                             int n_points) {
    matd_t *I3 = matd_identity(3);
    ...
    matd_t *R_z = matd_create_data(
        3, 3,
        (double[]){r31 / hypotenuse, -r32 / hypotenuse, 0, r32 / hypotenuse,
    ...
    matd_t *M2 =
        matd_create_data(3, 3, (double[]){-1, 0, 0, 0, 1, 0, 0, 0, -1});
    ...
    for (int i = 0; i < n_points; i++) {
        matd_t *op_tmp1 =
            matd_op("(M-M)MM", F_trans[i], I3, R_gamma, p_trans[i]);
        matd_t *op_tmp2 =
    ...
    for (int i = 0; i < n_points; i++) {
        ...
        matd_t *c1 = matd_op("(M-M)(MMM+M)", I3, F_trans[i], R_gamma, M1,
                             p_trans[i], b1_);
        matd_t *c2 = matd_op("(M-M)(MMM+M)", I3, F_trans[i], R_gamma, M2,
                             p_trans[i], b2_);
    ...
    for (int i = 0; i < n_roots; i++) {
        ...
        if (a2 - 2 * a0 + (3 * a3 - 6 * a1) * t1 +
                ...
                (-6 * a4 + 3 * a2) * t4 + a3 * t5 >=
            0) {
            // And that it corresponds to an angle different than the known
            // minimum.
            double t_cur = 2 * atan(roots[i]);
            // We only care about finding a second local minima which is
            // qualitatively different than the first.
...
void estimate_pose_for_tag_homography(apriltag_detection_info_t *info,
    ...
    matd_t *M_H = homography_to_pose(info->det->H, -info->fx, info->fy,
                                     info->cx, info->cy);
...
void estimate_tag_pose_orthogonal_iteration(
    ...
    *err1 = orthogonal_iteration(v, p, &solution1->t, &solution1->R, 4, nIters);
    ...
    if (solution2->R) {
        ...
        *err2 =
            orthogonal_iteration(v, p, &solution2->t, &solution2->R, 4, nIters);
    } else {
        *err2 = HUGE_VAL;
    }
