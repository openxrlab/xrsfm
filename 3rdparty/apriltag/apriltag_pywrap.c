...
#define SET_SIGINT()                                                           \
    ...
    do {                                                                       \
        if (0 != sigaction(SIGINT, &(struct sigaction){.sa_handler = SIG_DFL}, \
                           &sigaction_old)) {                                  \
...
#define PYMETHODDEF_ENTRY(function_prefix, name, args)                         \
    {                                                                          \
#name, (PyCFunction)function_prefix##name, args,                       \
...
static PyObject *apriltag_new(PyTypeObject *type, PyObject *args,
                              PyObject *kwargs) {
    bool success = false;
    ...
    char *keywords[] = {"family", "threads",      "maxhamming", "decimate",
                        "blur",   "refine-edges", "debug",      NULL};

    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "s|iiffOO", keywords,
                                     &family, &Nthreads, &maxhamming, &decimate,
                                     &blur, &py_refine_edges, &py_debug)) {
        goto done;
    }
    ...
    if (0)
        ;
    SUPPORTED_TAG_FAMILIES(TAG_CREATE_FAMILY);
    ...
    case EINVAL:
        PyErr_SetString(PyExc_RuntimeError,
                        "Unable to add family to detector. \"maxhamming\" "
                        "parameter should not exceed 3");
        break;
    case ENOMEM:
        PyErr_Format(
            PyExc_RuntimeError,
            "Unable to add family to detector due to insufficient memory to "
            "allocate the tag-family decoder. Try reducing \"maxhamming\" from "
            "%d or choose an alternative tag family",
            maxhamming);
        break;
    default:
        success = true;
...
static PyObject *apriltag_detect(apriltag_py_t *self, PyObject *args) {
    ...
    if (N == 0 && errno == EAGAIN) {
        PyErr_Format(PyExc_RuntimeError,
                     "Unable to create %d threads for detector",
                     self->td->nthreads);
        goto done;
    }
    ...
    for (int i = 0; i < N; i++) {
        xy_c = (PyArrayObject *)PyArray_SimpleNew(1, ((npy_intp[]){2}),
                                                  NPY_FLOAT64);
        if (xy_c == NULL) {
            PyErr_SetString(PyExc_RuntimeError,
        ...
        xy_lb_rb_rt_lt = (PyArrayObject *)PyArray_SimpleNew(
            2, ((npy_intp[]){4, 2}), NPY_FLOAT64);
        ...
        *(double *)PyArray_GETPTR1(xy_c, 1) = det->c[1];
        ...
        PyTuple_SET_ITEM(
            detections_tuple, i,
            Py_BuildValue("{s:i,s:f,s:i,s:N,s:N}", "hamming", det->hamming,
                          "margin", det->decision_margin, "id", det->id,
                          "center", xy_c, "lb-rb-rt-lt", xy_lb_rb_rt_lt));
        xy_c = NULL;
        xy_lb_rb_rt_lt = NULL;
    }
...
static PyTypeObject apriltagType = {PyVarObject_HEAD_INIT(NULL, 0).tp_name =
                                        "apriltag",
...
PyMODINIT_FUNC initapriltag(void) {
    ...
    PyObject *module = Py_InitModule3(
        "apriltag", methods, "AprilTags visual fiducial system detector");
...
static struct PyModuleDef module_def = {
    PyModuleDef_HEAD_INIT, "apriltag",
    "AprilTags visual fiducial system detector", -1, methods};
